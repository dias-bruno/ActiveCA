...
```{r}
gss_m_2015 <- gss_m_2015[,c('PUMFID','LUC_RST')]

walking_2015 <- walking_2015 |> 
  left_join(gss_m_2015, by = 'PUMFID') |>  # Merging the two tables
  filter(LUC_RST == 1) # Selecting only respondents from CMA/CA
```
...
Now, let's select only the cases of respondents who live in a CMA or CA:
```{r}
gss_m_2010 <- gss_m_2010[,c('RECID','LUC_RST')]
...
Now, let's select only the cases of respondents who live in a CMA or CA:
```{r}
gss_m_2005 <- gss_m_2005[,c('RECID','LUC_RST')]
...
Now, let's select only the cases of respondents who live in a CMA or CA:
```{r}
gss_m_1998 <- gss_m_1998[,c('RECID','CMAPRV')]

walking_1998 <- walking_1998 |> 
  left_join(gss_m_1998, by = c( 'PUMFID' = 'RECID')) |>  # Merging the two tables
  filter(CMAPRV %in% c(6,8,11,13,15)) # Selecting only respondents from CMA/CA
```
...
Now, let's select only the cases of respondents who live in a CMA or CA:
```{r}
gss_m_1992 <- gss_m_1992[,c('SEQNUM','DVCMA')]

walking_1992 <- walking_1992 |> 
  left_join(gss_m_1992, by = c( 'PUMFID' = 'SEQNUM')) |>  # Merging the two tables
  filter(DVCMA %in% c(6,8,11,13,15)) # Selecting only respondents from CMA/CA
```
...
gss_episodes <- rbind(walking_2015[,1:13], cycling_2015[,1:13], walking_2010[,1:13], cycling_2010[,1:13], walking_2005[,1:13], cycling_2005[,1:13], cycling_1998[,1:13], walking_1998[,1:13], walking_1992[,1:13], cycling_1992[,1:13], walking_1986[,1:13])
...
gss_episodes <- gss_episodes |>
  mutate(dest_label = case_when(
    ...
    dest_label == "Library, museum or theatre" ~ "Library",
