...
[**ActiveCA**](https://github.com/dias-bruno/ActiveCA) package. The main
objective of the *ActiveCA* package is to identify the most appropriate
impedance functions for active transportation modes for various
destinations and time periods in Canada. To do achieve this objective,
we analyzed historical data from the [General Social Survey
...
1986, with the most recent being in 2022 (1986, 1992, 1998, 2005, 2010,
2015, and 2022). The time use collects information on participation of
...
| Origin/Destination                     | LOCATION (2022)        | LOCATION (2015) | PLACE (2010) | PLACE (2005) | PLACE (1998) | PLACE (1998) | LOCATION (1986) |
| -------------------------------------- | ---------------------- | --------------- | ------------ | ------------ | ------------ | ------------ | --------------- |
| Home                                   | 3300                   | 300             | 1            | 1            | 1            | 1            | 1               |
| Work or school                         | 3301                   | 301             | 2, 8         | 2, 8         | 2            | 2            | 2               |
| Business                               | 3302                   | 302             |              |              |              |              |                 |
| Other's home                           | 3303                   | 303             | 3            | 3            | 3            | 3            |                 |
| In the neighbourhood                   | 3304                   | 304             |              |              |              |              |                 |
| Outdoors                               | 3305                   | 305             | 9            | 9            |              |              |                 |
| Grocery store, other stores or mall    | 3306                   | 306             | 6, 7         | 6, 7         |              |              |                 |
| Library, museum or theatre             | 3307                   | 307             | 10           | 10           |              |              |                 |
| Sports centre, field or arena          | 3308                   | 308             |              |              |              |              |                 |
| Restaurant, bar or club                | 3309                   | 309             | 4            | 4            |              |              |                 |
| Place of worship                       | 3310                   | 310             | 5            | 5            |              |              |                 |
| Medical, dental or other health clinic | 3311                   | 311             |              |              |              |              |                 |
| Elsewhere                              | 3312                   | 312             | 11           | 11           | 4            | 4            | 3               |
| Travel - Motorized                     | 3313, 3314, 3319, 3320 | 313, 314, 319   | 12, 13, 19   | 12, 13, 19   | 5, 6         | 5, 6         | 4               |
| Travel - Public transit                | 3316                   | 316             | 15, 16       | 15, 16       | 8            | 8            | 6               |
| Travel - Walk                          | 3315                   | 315             | 14           | 14           | 7            | 7            | 5               |
| Travel - Bicycle                       | 3318                   | 318             | 17           | 17           | 9            | 9            |                 |
| Travel - Other                         |  3317, 3323, 3399      | 317, 320, 321   | 18, 20, 21   | 18, 20, 21   | 10           | 10           | 7               |
...
-   `PUMFID`: the record identification[^1].
...
[^1]: This variable serves as the link between the Main and the Episode
...
The file name pattern is as follows: `transportation mode`[^2] + `_` +
`year of survey` [^3]. This means that, for instance, the file
`walking_2015` refers to transportation mode table `walking` in the year
2015.

[^2]: `walking` or `cycling`

[^3]: whether 1986, 1992, 1998, 2005, 2010, 2015, or 2022. Note: There
    is no data on cycling for the 1986 GSS survey.
...
selecting the columns that we are interested[^4], and already creating
the origin and destination variables:

[^4]: Specifically for the 2022 GSS survey, `TUI_01` refers to the
...
walking_2015 <- walking_2015 |>
  filter(!origin %in% c(315, 318, 317,320,321)) |>
  mutate(orig_label =
           case_when(origin == 300 ~ "Home", 
                     ...
                     origin == 312 ~ "Elsewhere",
                     origin %in% c(313,314,319) ~ "Travel - Motorized",
                     origin == 316 ~ "Travel - Transit"),
         orig_label = factor(orig_label)) %>%   filter(!is.na(orig_label))
```
...
```{r}
walking_2015 <- walking_2015 |>  
  filter(!destination %in% c(315, 318, 317,320,321)) |>
  mutate(dest_label =
           case_when(destination == 300 ~ "Home", 
                     ...
                     destination == 312 ~ "Elsewhere",
                     destination %in% c(313,314,319) ~ "Travel - Motorized",
                     destination == 316 ~ "Travel - Transit"),
         dest_label = factor(dest_label)) %>%   filter(!is.na(dest_label))
```
...
The table above shows that the most common destination for a walking
trip that starts at home are `Home` and `Work or school`. It also shows
that the second most common walking trip is from
`Grocery store, other stores or mall` to `Home`. Notice, though, that
quite a few trips start *and* end at Home. What are those supposed to
be?
...
[^5]: `TUI_01` is an activity code of the episode (What were you doing
...
Label the activity ("7" is "Transport to or from activity") and drop the
LOCATION because it is already coded as MODE:
...
`Work or school`. Again, we can notice that there are quite a few trips
that start *and* end at Home. What are those supposed to be?
...
= `2`) before cycling for approximately 10 minutes to a grocery store.
After spending 30 minutes at the store, they cycled back home. Upon
returning, the person prepared a meal and cleaned the house for
approximately 85 minutes. Subsequently, the individual embarked on
another bicycle trip to the store, which took 10 minutes, and thereafter
returned home.[^6]

[^6]: Note: `TUI_01` is an activity code of the episode (What were you
...
cycling_2015 <- cycling_2015 |>
  mutate(act_label = "Transport to or from activity") |>
  dplyr::select(PUMFID, WGHT_EPI, ACTCODE, act_label, STARTIME, ENDTIME, DURATION, origin, destination, orig_label, dest_label, YEAR, MODE, LUC_RST, PRV, Pop_centre, Province) %>%    distinct()
```
...
As we have already processed this year's data, we will clear the Episode
and Main files for this year from the computer's memory before uploading
any more data
...
walking_2010 <- walking_2010 |>
  filter(!origin %in% c(18, 20, 21, 14, 17)) |>
  mutate(orig_label =
           case_when(origin == 1 ~ "Home", 
                     origin %in% c(2,8) ~ "Work or school",
                     origin == 3 ~ "Other's home",
                     origin == 9 ~ "Outdoors",
                     origin %in% c(6,7) ~ "Grocery store, other stores or mall",
                     origin == 10 ~ "Library, museum or theatre",
                     origin == 4 ~ "Restaurant, bar or club",
                     origin == 5 ~ "Place of worship",
                     origin == 11 ~ "Elsewhere",
                     origin %in% c(12,13,19) ~ "Travel - Motorized",
                     origin %in% c(15,16) ~ "Travel - Transit"),
         orig_label = factor(orig_label)) %>%   filter(!is.na(orig_label))
```
...
```{r}
walking_2010 <- walking_2010 |>
  filter(!destination %in% c(18, 20, 21, 14, 17)) |>
  mutate(dest_label =
           case_when(destination == 1 ~ "Home", 
                     destination %in% c(2,8) ~ "Work or school",
                     destination == 3 ~ "Other's home",
                     destination == 9 ~ "Outdoors",
                     destination %in% c(6,7) ~ "Grocery store, other stores or mall",
                     destination == 10 ~ "Library, museum or theatre",
                     destination == 4 ~ "Restaurant, bar or club",
                     destination == 5 ~ "Place of worship",
                     destination == 11 ~ "Elsewhere",
                     destination %in% c(12,13,19) ~ "Travel - Motorized",
                     destination %in% c(15,16) ~ "Travel - Transit"),
         dest_label = factor(dest_label)) %>%   filter(!is.na(dest_label))
```
...
For the year 2010, the most common destination for a walking trip that
starts at home are `Home` to `Home`, followed by `Home` to
`Work or school`. Again, we can check the home-to-home trips:
...
activities related to washing and dressing (`ACTCODE`=`400`) for around
10 minutes, then went for a 10-minute walk, returning home after that.
Subsequently, they dedicated 10 minutes to meal preparation
(`ACTCODE`=`101`), followed by a 10-minute session of food (or meal)
cleanup (`ACTCODE`=`110`). The individual then took a 15-minute walk,
returned home, and spent 5 minutes on household management tasks, which
included organizing and planning activities.[^7]

[^7]: Note: `ACTCODE` is an activity code of the episode (What were you
    doing in minute).The list of main activity codes can be found in an
    appendix of the User Guide.

Label the activity (see the activity codes in the 2010 User's Guide) and
drop the LOCATION because it is already coded as MODE:
...
(`ACTCODE`= 400) before embarking on a brief walk lasting approximately
5 minutes, after which they returned home. Subsequently, the person
...
labeled as `ACTCODE` = 11 for 465 minutes, which denotes working from
home as their primary workplace. Following this work period, the person
went out for another 15-minute cycling session, then returned home once
more and spent 90 minutes watching television.
...
walking_1998 <- walking_1998 |>
  filter(!origin %in% c(7, 9, 10)) |>
  mutate(orig_label =
           case_when(origin == 1 ~ "Home", 
                     origin == 2 ~ "Work or school",
                     origin == 3 ~ "Other's home",
                     origin == 4 ~ "Elsewhere",
                     origin %in% c(5,6) ~ "Travel - Motorized",
                     origin == 8  ~ "Travel - Transit"),
         orig_label = factor(orig_label)) %>%   filter(!is.na(orig_label))
```
...
```{r}
walking_1998 <- walking_1998 |>
  filter(!destination %in% c(7, 9, 10)) |>
  mutate(dest_label =
           case_when(destination == 1 ~ "Home", 
                     destination == 2 ~ "Work or school",
                     destination == 3 ~ "Other's home",
                     destination == 4 ~ "Elsewhere",
                     destination %in% c(5,6) ~ "Travel - Motorized",
                     destination == 8  ~ "Travel - Transit"),
         dest_label = factor(dest_label)) %>%   filter(!is.na(dest_label))
```
...
In this case, the most common destination for a walking trip that starts
at home are `other's Home`.
...
The person was at home doing food cleanup (`ACTCODE` = 110), then went
for walking that lasted 30 minutes, came back home and spent 15 minutes
relaxing/resting. Here have here another case of a recreational/leisure
trip!
...
The person spent 165 minutes talking to someone (in person or on the
phone, `ARTCODE` = 950), then embarked on a bike ride for 12 minutes.
After that, the person spent 108 minutes shopping at home (`ARTCODE` =
380), cycled for another 12 minutes and came back to talk to someone for
another 138 minutes (`ARTCODE` = 950).
...
The person was at home reading newspapers (`ACTCODE` = 940), then went
for walking that lasted 30 minutes, came back home and spent 90 minutes
resting (`ACTCODE` = 470).[^9]
...
walking_1992 <- walking_1992 |>
  mutate(act_label = case_when(ACTCODE == 2 ~ "Refused information",
                               ...
                               ACTCODE == 302 ~ "Everyday shopping: everyday goods and products (clothing, gas, etc.)", # p. 188
                               ACTCODE == 303 ~ "Everyday shopping: Take-out food", # p. 188
                               ACTCODE == 331 ~ "Financial Services (e.g. banking)", # p. 188
                               ACTCODE == 332 ~ "Government Services (e.g. UIC)", # p. 188
                               ACTCODE == 370 ~ "Waiting for purchases or services", # p. 188
                               ACTCODE == 380 ~ "Other Shopping and Services", # p. 188
                               ACTCODE == 390 ~ "Travel to/from shopping or obtaining services", # p. 188
                               ACTCODE == 400 ~ "Washing, Dressing", # p. 202
                               ACTCODE == 470 ~ "Relaxing, Thinking, Resting", # p. 202
                               ACTCODE == 480 ~ "Other Personal Care or Private Activities", # p. 202
                               ACTCODE == 491 ~ "Travel to/from restaurant meals", # p. 202
                               ACTCODE == 492 ~ "Travel to/from other personal care activities", # p. 202
                               ACTCODE == 550 ~ "Breaks/Waiting for Class", # p. 208
                               ACTCODE == 580 ~ "Other Study", # p. 208
                               ACTCODE == 590 ~ "Travel to/from school education activities", # p. 208
                               ACTCODE == 610 ~ "Political, civic activity (e.g. voting, jury duty, city council, donating blood)", # p. 215
                               ACTCODE == 640 ~ "Religious ServicesfPrayerfRead Bible", # p. 215
                               ACTCODE == 673 ~ "Unpaid babysitting", # p. 215
                               ACTCODE == 674 ~ "Transportation assistance to someone other than a household member", # p. 215
                               ACTCODE == 678 ~ "Other unpaid work ", # p. 215
                               ACTCODE == 680 ~ "Other Organizational, Voluntary and Religious Activity", # p. 215
                               ACTCODE == 691 ~ "Travel to/from civic and voluntary activity", # p. 216
                               ACTCODE == 692 ~ "Travel to/from religious services", # p. 216
                               ACTCODE == 753 ~ "Socializing w/friends (non-private residence)", # p. 234
                               ACTCODE == 780 ~ "Other Social Gatherings (Weddings, Wakes)", # p. 234
                               ACTCODE == 791 ~ "Travel to/from attending sports, movies or other entertainment events or visit sites", # p. 234
                               ACTCODE == 792 ~ "Travel to/from socializing at private residences", # p. 234
                               ACTCODE == 793 ~ "Travel to/from other socializing (to bars, hospitals, weddings)", # p. 234
                               ACTCODE == 814 ~ "Camping", # p. 244
                               ACTCODE == 816 ~ "Other outdoor activities/excursions (picnic, car rally, bird watching)", # p. 244
                               ACTCODE == 821 ~ "Walking, hiking, jogging, running", # p. 821
                               ACTCODE == 822 ~ "Bicycling", # p. 244
                               ACTCODE == 831 ~ "Hobbies done mainly for pleasure (painting, sketching, photography)", # p. 244
                               ...
                               ACTCODE == 893 ~ "Travel to/from hobbies or for the sale of crafts", # p. 245
                               ACTCODE == 894 ~ "Travel to/from other leisure activities", # p. 245
                               ACTCODE == 950 ~ "Talking, Conversation, Phone", # p. 266
                               ACTCODE == 961 ~ "Reading personal mail (including flyers and advertisements)", # p. 266
                               ACTCODE == 990 ~ "Travel to/from media or communication activities")) |> # p. 266 
  dplyr::select(PUMFID, WGHT_EPI, ACTCODE, act_label, STARTIME, ENDTIME, DURATION, origin, destination, orig_label, dest_label, YEAR, MODE, LUC_RST, PRV, Pop_centre, Province) %>%    distinct()
```

Cases of episodes with `NA` weight values will be updated to value 1,
meaning that since there's no weighting value for this observation, this
episode only represent itself:
...
The person was at home Talking or having conversation on the phone
(`ACTCODE` = 950), then went for cycling (`ACTCODE` = 822) that lasted
60 minutes, came back home and spent 15 minutes washing and dressing
(`ACTCODE` = 400).
...
cycling_1992 <- cycling_1992 |>
  mutate(act_label = case_when(ACTCODE == 2 ~ "Refused information",
                               ...
                               ACTCODE == 872 ~ "Pleasure drives (as a passenger in the car)", # p. 245
                               ACTCODE == 873 ~ "Other pleasure drives (e.g. on a tour bus)", # p. 245
                               ACTCODE == 880 ~ "Other Sport or Active Leisure", # p. 245
                               ACTCODE == 891 ~ "Travel to/from participation in active sport/outdoor activities", # p. 245
                               ACTCODE == 892 ~ "Travel to/from coaching activities", # p. 245
                               ACTCODE == 893 ~ "Travel to/from hobbies or for the sale of crafts", # p. 245
                               ACTCODE == 894 ~ "Travel to/from other leisure activities", # p. 245
...
walking_1986 <- walking_1986 |>
  filter(!origin %in% c(5,7)) |>
  mutate(orig_label =
           case_when(origin == 1 ~ "Home", 
                     origin == 2 ~ "Work or school",
                     origin == 3 ~ "Elsewhere",
                     origin == 4 ~ "Travel - Motorized",
                     origin == 6 ~ "Travel - Transit"),
         orig_label = factor(orig_label)) %>%   filter(!is.na(orig_label))
```
...
walking_1986 <- walking_1986 |>
  ...
  mutate(dest_label =
           case_when(destination == 1 ~ "Home", 
                     destination == 2 ~ "Work or school",
                     destination == 3 ~ "Elsewhere",
                     destination == 4 ~ "Travel - Motorized",
                     destination == 6 ~ "Travel - Transit"),
         dest_label = factor(dest_label)) %>%   filter(!is.na(dest_label))
```
...
**IMPORTANT:** See p. A96 of the User's Guide for Cycle 2. The
`WGHT_EPI` values, which give the number of people a record represents,
(in this case the `FWGT_MS`) are given as a 9 digit integer with 4
decimals, like so: XXXXX.XXXX. We cannot just copy `FWGT_MS` to
`WGHT_EPI` below, we must divide first by 10,000:
...
The person was at home listening radio for 30 minutes (`ARTCODE` = 90),
then went for a walk for gardening/pet care (`ACT_CODE` = 17) that
lasted 15 minutes, came back home and spent 90 minutes resting/relaxing
(`ARTCODE` = 47).
...
walking_1986 <- walking_1986 |>
  mutate(act_label = case_when(ACTCODE == 1 ~ "Work for Pay",
                               ...
                               ACTCODE == 4 ~ "Waiting/Delays at Work", # p. 156
                               ...
                               ACTCODE == 14 ~ "Laundry, Ironing, Folding", # p. 164
                               ...
                               ACTCODE == 24 ~ "Play with Children", # p. 177
                               ...
                               ACTCODE == 32 ~ "Personal Care Services", # p. 188
                               ...
                               ACTCODE == 51 ~ "Other classes – part-time", # p. 208 
                               ...
                               ACTCODE == 92 ~ "Listening to CD’s, cassette tapes or records", # p. 266
                               ACTCODE == 93 ~ "Reading books, magazines", # p. 266
                               ACTCODE == 94 ~ "Reading Newspapers", # p. 266
...
Now, we'll select all the rows with walking episodes in the GSS survey,
selecting the columns that we are interested[^10], and already creating
the origin and destination variables:

[^10]: Specifically for the 2022 GSS survey, `TUI_01` refers to the
    activity code. We'll change the name of this column to be match the
    name of the activity code variable in the others surveys.
...
walking_2022_spss <- walking_2022_spss |>
  filter(!is.na(origin)) |>
  filter(!origin %in% c('Travel - Airplane', 
                                   'Travel - Other',
                                   'Travel - not stated', 
                        'Travel - Walk', 'Travel - Bicycle')) %>% 
  mutate(orig_label =
           case_when(origin == 'At home or on property' ~ "Home", 
                     ...
                     origin %in% c('Travel - Car, truck or van,  - as passenger', 
                                   'Travel - Car, truck or van,  - as driver',
                                   'Travel - Motorcycle, scooter or moped',
                                   'Travel - Taxi, Limousine service') ~ "Travel - Motorized", 
                     origin == 'Travel - Public transit (bus, streetcar, subway, light-rail)' ~ "Travel - Transit",
                     TRUE ~ origin),
         orig_label = factor(orig_label)) %>%   filter(!is.na(orig_label))
```
...
walking_2022_spss <- walking_2022_spss |>
  ...
  filter(!destination %in% c('Travel - Airplane', 
                                   'Travel - Other',
                                   'Travel - not stated', 
                        'Travel - Walk', 'Travel - Bicycle')) %>% 
    mutate(dest_label =
           case_when(destination == 'At home or on property' ~ "Home", 
                     ...
                     destination %in% c('Travel - Car, truck or van,  - as passenger', 
                                   'Travel - Car, truck or van,  - as driver',
                                   'Travel - Motorcycle, scooter or moped',
                                   'Travel - Taxi, Limousine service') ~ "Travel - Motorized", 
                     destination == 'Travel - Public transit (bus, streetcar, subway, light-rail)' ~ "Travel - Transit",
                     TRUE ~ destination),
         dest_label = factor(dest_label)) %>%   filter(!is.na(dest_label))
```
...
numeric_columns <- c("WGHT_EPI")

int_columns <- c("PUMFID", "ACTCODE", "STARTIME", "ENDTIME", "DURATION",
                     "origin", "destination", "LUC_RST",  "PRV")

for (col in numeric_columns) {
  walking_2022[[col]] <- as.numeric(as.character(walking_2022[[col]]))
}

for (col in int_columns) {
  walking_2022[[col]] <- as.integer(as.character(walking_2022[[col]]))
}
```
...
The person was at home doing regular household tasks, then went for walk
that lasted 20 minutes, came back home and returned to the household
tasks for more 180 minutes.
...
Now, we'll select all the rows with cycling episodes in the GSS survey,
selecting the columns that we are interested[^11], and already creating
the origin and destination variables:

[^11]: Specifically for the 2022 GSS survey, `TUI_01` refers to the
    activity code. We'll change the name of this column to be match the
    name of the activity code variable in the others surveys.
...
for (col in numeric_columns) {
  cycling_2022[[col]] <- as.numeric(as.character(cycling_2022[[col]]))
}

for (col in int_columns) {
  cycling_2022[[col]] <- as.integer(as.character(cycling_2022[[col]]))
}
```
...
The table above shows that the most common destination for a cycling
trip that starts at home are `Home` and `Work or school`. It also shows
that the second most common cycling trip is from
`Grocery store, other stores or mall` to `Home`. Notice, though, that
quite a few trips start *and* end at Home. What are those supposed to
be?
...
The person was at home doing
`Culture, sports events, hobbies, leisure or outdoor activity`, then
went for cycling trip that lasted 60 minutes, came back home and spent
120 minutes `reading, whatching TV or listening music`. So, this was a
recreational/leisure trip!
