---
title: "Reading main files and preparing data sets"
author: "Bruno Santos, Mahdis Moghadasi & Antonio Paez"
date: "`r Sys.Date()`"
output:
  tufte::tufte_html: default
  tufte::tufte_handout:
    citation_package: natbib
    latex_engine: xelatex
  tufte::tufte_book:
    citation_package: natbib
    latex_engine: xelatex
bibliography: activeca.bib
link-citations: yes
editor_options: 
  markdown: 
    wrap: 72
---

```{r setup, include=FALSE}
# layout configuration 
library(tufte)
knitr::opts_chunk$set(warning = FALSE, message = FALSE)
options(htmltools.dir.version = FALSE)
rm(list=ls())
```

# Introduction

This Rmarkdown file is part of the
[**ActiveCA**](https://github.com/dias-bruno/ActiveCA) package. The main objective of the *ActiveCA* package is to identify the most
appropriate impedance functions for active transportation modes for
various destinations and time periods in Canada. To do achieve this objective, we analyzed historical data from the [General Social Survey
(GSS)](https://www23.statcan.gc.ca/imdb/p2SV.pl?Function=getSurvey&SDDS=5221)
from 1986 to 2015 in Canada to calculate the impedance function for
cycling and walking trips.

In this R markdown, we read the main files of GSS surveys and prepare the data set to
the following analyses.

# The General Social Surveys (GSS)

In Canada, Statistics Canada produces the General Social Surveys (GSS)
to collect data on social trends in order to track changes in the living
conditions of Canadians and well-being over time. This survey tracks
changes in time and it is used to understand how Canadians spend and
manage their time and what factors contribute to their happiness and
stress. The survey was created in 1985 as a series of independent,
annual, cross-sectional surveys, each covering one topic in depth.
Caregiving, families, time use, social identity, volunteering, and
victimization are among the current themes of the GSS.

The six survey themes listed above are repeated every five years. In
addition to the central topic, each cycle includes new content that
addresses emerging and policy-relevant issues, collecting detailed
socio-demographic information, such as age, gender, education, religion,
ethnicity, income, etc. Time-use surveys collect data on all human
activities and can inform a wide range of policies. In particular, three
key themes have been identified as essential for policymaking and for
which no other data source is adequate: unpaid work and non-market
production, well-being, and gender equality. Leisure time, work-life
balance, health, commuting, culture, and sports are among the other
topics covered by time-use surveys. In addition, statistics Canada has
been conducting time-use surveys at five to seven year intervals since
1986, with the most recent being in 2022 (1986, 1992, 1998, 2005, 2010, 2015, and 2022). The time use collects information on participation of
respondents and time spent on a wide range of day-to-day activities
using a 24-hour retrospective diary. Also, information is collected on
the location of these activities (e.g., at home, at work, etc.) and, for
non-personal activities, the people who were present with the respondent
at the time of the activity.

The surveys (obtained from
[here](http://odesi2.scholarsportal.info/webview/)) consist of two
files: a main file and an episode file.

## The Main File

The Main File compiles an large array of aggregated data, summarizing
the answers to the questionnaire and derived variables that summarize
the respondents' time use across different activities, locations, and
social interactions. This file documents the time and duration that
respondents allocate to each activity and location. The Main File
provides a overview of daily routines and social dynamics, not focusing
on individual activity episodes. Additionally, this file categorizes
activities into bigger groups and subcategories, enhancing the data's
analytical utility with additional metrics such as total transit time,
duration spent with household members, and counts of activities and
episodes. This file is aggregated and prohibits the direct association
between specific activities with particular locations or companions.

# Reading and processing the GSS files

In this R markdown, our aim is to generate a table in the appropriate
format to perform the subsequent analyses, after reading and processing
the GSS main surveys.

To start processing the data, first load the `R` packages used in this
notebook:

```{r read-libraries}
library(dplyr) # A Grammar of Data Manipulation
library(ggplot2) # Create Elegant Data Visualisations Using the Grammar of Graphics
library(here) # A Simpler Way to Find Your Files
library(readxl) # Read Excel Files
library(tidyr) # Tidy Messy Data
library(usethis) # Automate Package and Project Setup
library(purrr) # Provides a complete and consistent set of tools for working with functions and vectors
library(foreign) # Read SPSS files
```

For each survey year, the process of preparing the data is very similar
and, in the end, we will obtain a table in which each row represents an
specific respondent, and each column a variable. Since every survey introduced different variables, 
we will not try to align the name columns and values between the tables. 

The file name pattern is as follows: `gss_main_` +
`year of survey` [^4]. This means that, for instance, the file
`gss_main_20105` refers to the main file of the 2005 GSS survey.

## GSS 2022

```{r reading-2022-main-file}
# Main File
gss_m_2022 <- read.spss(paste0(here(), "/data-raw/source-files/Time-use-2022/gss_tu_main_2022_pumf_v1.sav"), to.data.frame = TRUE) # Original file

gss_m_2022_b <- gss_m_2022
gss_m_2022 <- gss_m_2022_b
```

The main data set for 2022 is already labeled and only needs a selection of socioeconomic and demographic variables:

```{r filtering-variables-2022}
gss_main_2022 <- gss_m_2022 %>%
  dplyr::select("PUMFID", "WGHT_PER","SURVMNTH", "PRV", "LUC_RST", "REGION",  "HSDSIZEC","AGEGR10", "GENDER2",
"MARSTAT", "PHSDFLG", "AGEPRC",  "GENPR2",  "COUPGEN2","CHH0017C","CHH0004C","CHH0514C","CHH1517C","CXRFLAG",
"PARNUM",  "SENFLAG", "MULTIGEN","DDEV_FL", "DDEX_FL", "DFLEX_FL","DHEAR_FL","DLRN_FL", "DMEM_FL", "DMENT_FL",
 "DMOB_FL", "DPAIN_FL","DUNK_FL", "DVIS_FL", "DDIS_FL", "DTYPER",  "DCLASS",  "TCS_110", "TCS_120", "TCS_130",
 "TCS_140", "TCS_150", "TCS_160", "TCS_170", "TCS_180", "TCS_190", "TCS_200",  "TIMECR", "GEN_01",  "GEN_02",  "SRS_10",  "MSS_130", "ACT7DAYC","MRW_05C", "MRW_D40A",
"MRW_D40B","WET_120",  "WHW_230", "WHWD140G","TLWK_01A","TLWK_06","WLYD170G",
"ATT_120", "ATT_130", "ATT_131",
"ATT_150C","ATT_160", "ATT_170",
"CTW_140A","CTW_140B","CTW_140C","CTW_140D",
"CTW_140E","CTW140GR","CTW_140I","CTW_150G","CTW_160", "CTW_170", "CTW_180", "CTW_190", "CTW_210", "CTW_215",
"ED_05",  "MAP_110C","MAP_D40A","DOR_110C","IM_01A",  "YRARRI_C","BPP_01A", "BPR_20", "ABM_01A",
"AIP_01A", "VISMINFL","VISPRFL", "REL_02",  "RELIGFLG","INC_C",  
"FAMINC_C","GTU_110", "GTU_130",  "DDAY",  "DVTDAY") %>% 
  mutate(PUMFID = as.numeric(as.character(PUMFID)),
         WGHT_PER = as.numeric(as.character(WGHT_PER)))
```

```{r LUC_RST-2022}
gss_main_2022 <- gss_main_2022 %>% 
  mutate( # LUC_RST:  Population centre indicator
    LUC_RST = 
      case_when(LUC_RST == "Larger urban population centres (CMA/CA)" ~ "CMA/CA",
                     LUC_RST == "Rural areas/small population centres (non-CMA/CA)" ~ "non CMA/CA",
                     LUC_RST == "Prince Edward Island" ~ "Prince Edward Island"),
    LUC_RST = factor(LUC_RST)) 

summary(gss_main_2022$LUC_RST)
```
```{r HSDSIZEC-2022}
values_ordered <- c("One person", 
           "Two persons",
           "Three persons",
           "Four persons",
           "Five persons or more")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# HSDSIZEC:  Household size of respondent (capped)
    HSDSIZEC = factor(HSDSIZEC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$HSDSIZEC)
```
```{r AGEGR10-2022}
values_ordered <- c("15 to 24 years","25 to 34 years","35 to 44 years",
                    "45 to 54 years", "55 to 64 years","65 to 74 years","75 years and over")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# AGEGR10:  Age group of respondent (groups of 10)
    AGEGR10 = factor(AGEGR10, levels = values_ordered, ordered = TRUE), 
    AGEPRC = factor(AGEPRC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$AGEGR10)
```
```{r AGEPRC-2022}
values_ordered <- c("15 to 34 years","35 to 44 years",
                    "45 to 54 years", "55 to 64 years","65 to 74 years","75 years and over")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# AGEPRC:  Age group of respondent’s spouse/partner (collapsed)
    AGEPRC = factor(AGEPRC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$AGEPRC)
```

```{r CHH0017C-2022}
summary(gss_main_2022$CHH0017C)

values_ordered <- c("No children","One child",
                    "Two children", "Three children or more")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# CHH0017C:  Number of child(ren) in household - 0 to 17 years (capped)
    CHH0017C = factor(CHH0017C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$CHH0017C)
```
```{r CHH0004C-2022}
summary(gss_main_2022$CHH0004C)

values_ordered <- c("No children","One or more children")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# CHH0004C:  Number of child(ren) in household - 0 to 4 years (capped)
    CHH0004C = factor(CHH0004C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$CHH0004C)
```

```{r CHH0514C-2022}
summary(gss_main_2022$CHH0514C)

values_ordered <- c("No children","One or more children")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# CHH0514C:  Child(ren) in household - 5 to 14 years (flag)
    CHH0514C = factor(CHH0514C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$CHH0514C)
```

```{r CHH1517C-2022}
summary(gss_main_2022$CHH1517C)

values_ordered <- c("No children","One or more children")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# CHH1517C:  Child(ren) in household - 15 to 17 years (flag)
    CHH1517C = factor(CHH1517C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$CHH1517C)
```

```{r PARNUM-2022}
summary(gss_main_2022$PARNUM)

values_ordered <- c("None","One parent","Two parents")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# PARNUM: Number of parents the respondent has in household
    PARNUM = factor(PARNUM, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$PARNUM)
```
```{r DTYPER-2022}
summary(gss_main_2022$DTYPER)

values_ordered <- c("Does not have a disability",
                    "Has one disability type",
                    "Has two or three disability types",
                    "Has more than three disability types")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# DTYPER: Disability type counter - Grouped
    DTYPER = factor(DTYPER, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$DTYPER)
```

```{r DCLASS-2022}
summary(gss_main_2022$DCLASS)

values_ordered <- c("Does not have a disability",
                    "Unknown severity",
                    "Mild",
                    "Moderate",
                    "Severe",
                    "Very severe")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# DCLASS: Global (disability) severity class
    DCLASS = factor(DCLASS, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$DCLASS)
```

```{r TIMECR-2022}
summary(gss_main_2022$TIMECR)

values_ordered <- c("Zero Yes codes",
                    "One Yes code",
                    "Two Yes codes",
                    "Three Yes codes",
                    "Four Yes codes",
                    "Five Yes codes",
                    "Six Yes codes",
                    "Seven Yes codes",
                    "Eight Yes codes",
                    "Nine Yes codes",
                    "Ten Yes codes")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# TIMECR: Time crunch indicator
    TIMECR = factor(TIMECR, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$TIMECR)
```

```{r GEN_01-2022}
summary(gss_main_2022$GEN_01)

values_ordered <- c("Poor",
                    "Fair",
                    "Good",
                    "Very good",
                    "Excellent")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# GEN_01: Self rated health - In general, how is your health?
    GEN_01 = factor(GEN_01, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$GEN_01)
```

```{r GEN_02-2022}
summary(gss_main_2022$GEN_02)

values_ordered <- c("Poor",
                    "Fair",
                    "Good",
                    "Very good",
                    "Excellent")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# GEN_02: Self rated mental health - In general, how is your mental health?
    GEN_02 = factor(GEN_02, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$GEN_02)
```

```{r SRS_10-2022}
summary(gss_main_2022$SRS_10)

values_ordered <- c("Not at all stressful",
                    "Not very stressful",
                    "A bit stressful",
                    "Quite a bit stressful",
                    "Extremely stressful")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# SRS_10: Self Rated Stress - Amount - Thinking about the amount of stress in your life, how would you describe most of your days?
    SRS_10 = factor(SRS_10, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$SRS_10)
```

```{r TLWK_06-2022}
summary(gss_main_2022$TLWK_06)

values_ordered <- c("1 - Not important at all",
                    "2",
                    "3",
                    "4",
                    "5 - Very important")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# TLWK_06: Telework aspects - Avoid the commute. How important is the opportunity not to commute every day to your workplace?
    TLWK_06 = factor(TLWK_06, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$TLWK_06)
```


```{r WLYD170G-2022}
summary(gss_main_2022$WLYD170G)

values_ordered <- c("Respondent works from home",
                    "0 to 4 kilometers",
                    "5 to 9 kilometers",
                    "10 to 14 kilometers",
                    "15 to 19 kilometers",
                    "20 to 24 kilometers",
                    "25 to 29 kilometers",
                    "30 to 39 kilometers",
                    "40 to 49 kilometers",
                    "50 kilometers or more")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# WLYD170G: Distance from residence to workplace (grouped)
    WLYD170G = factor(WLYD170G, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$WLYD170G)
```

```{r ATT_120-2022}
summary(gss_main_2022$ATT_120)

values_ordered <- c("All the time",
                    "Some of the time",
                    "Rarely",
                    "Never")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# ATT_120: Vehicle at respondent’s disposal. How often do you have a vehicle at your disposal?
    ATT_120 = factor(ATT_120, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$ATT_120)
```

```{r ATT_130-2022}
summary(gss_main_2022$ATT_130)

values_ordered <- c("All the time",
                    "Some of the time",
                    "Rarely",
                    "Never")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# ATT_130: Need someone else to drive - Frequency. How often do you need someone to drive you somewhere for an appointment, errand or something else?
    ATT_130 = factor(ATT_130, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$ATT_130)
```

```{r ATT_131-2022}
summary(gss_main_2022$ATT_131)

values_ordered <- c("All the time",
                    "Some of the time",
                    "Rarely",
                    "Never")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# ATT_131: Someone else can drive when needed - Frequency. How often can someone drive you when you need it?
    ATT_131 = factor(ATT_131, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$ATT_131)
```
```{r ATT_170-2022}
summary(gss_main_2022$ATT_170)

values_ordered <- c("Not in past 12 months",
                    "A few times a year",
                    "A few times a month",
                    "Several times a week",
                    "Every day")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# ATT_170: Use of public transportation - Past 12 months.
    ATT_170 = factor(ATT_170, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$ATT_170)
```

```{r ED_05-2022}
summary(gss_main_2022$ED_05)

values_ordered <- c("Less than high school diploma or its equivalent",
                    "High school diploma or a high school equivalency certificate",
                    "Trades certificate or diploma",
                    "College, CEGEP or other non-university certificate or diplom",
                    "University certificate or diploma below the bachelor's level",
                    "Bachelor's degree",
                    "University certificate, diploma, or degree above the bachelo")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# ED_05: Educational attainment - Highest degree (7 categories)
    ED_05 = factor(ED_05, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$ED_05)
```

```{r REL_02-2022}
summary(gss_main_2022$REL_02)

values_ordered <- c("Not at all",
                    "Once or twice a year",
                    "At least three times a year",
                    "At least once a month",
                    "At least once a week")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# REL_02: Frequency of religious participation - Past 12 months.
    REL_02 = factor(REL_02, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$REL_02)
```
```{r INC_C-2022}
summary(gss_main_2022$INC_C)

values_ordered <- c("Less than $25,000",
                    "$25,000 to $49,999",
                    "$50,000 to $74,999",
                    "$75,000 to $99,999",
                    "$100,000 and over")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# INC_C: Income - Total (before tax) (grouped).
    INC_C = factor(INC_C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$INC_C)
```

```{r FAMINC_C-2022}
summary(gss_main_2022$FAMINC_C)

values_ordered <- c("Less than $50,000",
                    "$50,000 to $74,999",
                    "$75,000 to $99,999",
                    "$100,000 to $124,999",
                    "$125,000 and over")

gss_main_2022 <- gss_main_2022 %>% 
  mutate(# FAMINC_C: Family Income - Total (before tax) (grouped).
    FAMINC_C = factor(FAMINC_C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2022$FAMINC_C)
```
```{r GTU_110-2022}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never")

gss_main_2022 <- gss_main_2022 %>% 
  mutate( # GTU_110: How often do you feel rushed? Would you say it is...?
    GTU_110 = factor(GTU_110, levels = rev(values_ordered), ordered = TRUE)) 

summary(gss_main_2022$GTU_110)
```

```{r GTU_130-2022}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never")

gss_main_2022 <- gss_main_2022 %>% 
  mutate( # GTU_130: How often do you feel you have time on your hands that you don’t know what to do with?
    GTU_130 = factor(GTU_130, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_2022$GTU_130)
```

Save data in R format for later use:
```{r save-data-2022}
usethis::use_data(gss_main_2022, 
                  overwrite = TRUE)
```

## GSS 2015

Read the Main file of the 2015 GSS survey:

```{r reading-2015-main-file}
# Main File
gss_m_2015 <- read.csv(paste0(here(), "/data-raw/source-files/Time-use-2015/gss-m.csv"))
```

Select only the socioeconomic variables: 

```{r filtering-variables-2015}
gss_main_2015 <- gss_m_2015 %>%
  select(PUMFID:DVTDAY, SLM_01, SRH_110, SRH_115)
```

Labeling socioeconomic variables: 

```{r SURVMNTH-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # SURVMNTH: Survey month of data collection from April 2015 to March 2016
    SURVMNTH = 
           case_when(SURVMNTH == 1 ~ "January", 
                     SURVMNTH == 2 ~ "February",
                     SURVMNTH == 3 ~ "March",
                     SURVMNTH == 4 ~ "April",
                     SURVMNTH == 5 ~ "May",
                     SURVMNTH == 6  ~ "June",
                     SURVMNTH == 7 ~ "July",
                     SURVMNTH == 8 ~ "August",
                     SURVMNTH == 9 ~ "September",
                     SURVMNTH == 10 ~ "October",
                     SURVMNTH == 11 ~ "November",
                     SURVMNTH == 12 ~ "December",
                     SURVMNTH == 96 ~ "Valid skip",
                     SURVMNTH == 97 ~ "Don't know",
                     SURVMNTH == 98 ~ "Refusal",
                     SURVMNTH == 99 ~ "Not stated"), 
    SURVMNTH = factor(SURVMNTH)) 

summary(gss_main_2015$SURVMNTH)
```

```{r AGEGR10-2015}
values_ordered <- c("15 to 24 years","25 to 34 years","35 to 44 years",
                    "45 to 54 years", "55 to 64 years","65 to 74 years","75 years and over")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # AGEGR10: Age group of respondent (groups of 10)
    AGEGR10 = 
           case_when(AGEGR10 == 1 ~ "15 to 24 years", 
                     AGEGR10 == 2 ~ "25 to 34 years",
                     AGEGR10 == 3 ~ "35 to 44 years",
                     AGEGR10 == 4 ~ "45 to 54 years",
                     AGEGR10 == 5 ~ "55 to 64 years",
                     AGEGR10 == 6  ~ "65 to 74 years",
                     AGEGR10 == 7 ~ "75 years and over",
                     AGEGR10 == 96 ~ "Valid skip",
                     AGEGR10 == 97 ~ "Don't know",
                     AGEGR10 == 98 ~ "Refusal",
                     AGEGR10 == 99 ~ "Not stated"), 
    AGEGR10 = factor(AGEGR10, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2015$AGEGR10)
```

```{r SEX-2015}
values_ordered <- c("Male", "Female")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # SEX: Sex of respondent
    SEX = 
           case_when(SEX == 1 ~ "Male", 
                     SEX == 2 ~ "Female",
                     SEX == 96 ~ "Valid skip",
                     SEX == 97 ~ "Don't know",
                     SEX == 98 ~ "Refusal",
                     SEX == 99 ~ "Not stated"), 
    SEX = factor(SEX, levels = values_ordered, ordered = FALSE)) 

summary(gss_main_2015$SEX)
```

```{r MARSTAT-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # MARSTAT: Marital status of the respondent
    MARSTAT = 
           case_when(MARSTAT == 1 ~ "Married", 
                     MARSTAT == 2 ~ "Living common-law",
                     MARSTAT == 3 ~ "Widowed",
                     MARSTAT == 4 ~ "Separated",
                     MARSTAT == 5 ~ "Divorced",
                     MARSTAT == 6  ~ "Single, never married",
                     MARSTAT == 96 ~ "Valid skip",
                     MARSTAT == 97 ~ "Don't know",
                     MARSTAT == 98 ~ "Refusal",
                     MARSTAT == 99 ~ "Not stated"), 
    MARSTAT = factor(MARSTAT))

summary(gss_main_2015$MARSTAT)
```


```{r AGEPRGRD-2015}
values_ordered <- c("Respondent is 11 + years older", 
                    "Respondent is 6 - 10 years older",
                    "Respondent is 5 years older",
                    "Respondent is 4 years older",
                    "Respondent is 3 years older",
                    "Respondent is 2 years older",
                    "Respondent is 1 years older",
                    "Respondent and spouse/partner are the same age",
                    "Respondent is 1 years younger",
                    "Respondent is 2 years younger",
                    "Respondent is 3 years younger",
                    "Respondent is 4 years younger",
                    "Respondent is 5 years younger",
                    "Respondent is 6 - 10 years younger",
                    "Respondent is 11 + years younger",
                    "Valid skip")
                    
                    
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # AGEPRGRD: Age difference between respondent and spouse/partner.
    AGEPRGRD = 
           case_when(AGEPRGRD == 1 ~ "Respondent is 11 + years older", 
                     AGEPRGRD == 2 ~ "Respondent is 6 - 10 years older",
                     AGEPRGRD == 3 ~ "Respondent is 5 years older",
                     AGEPRGRD == 4 ~ "Respondent is 4 years older",
                     AGEPRGRD == 5 ~ "Respondent is 3 years older",
                     AGEPRGRD == 6  ~ "Respondent is 2 years older",
                     AGEPRGRD == 7  ~ "Respondent is 1 years older",
                     AGEPRGRD == 8  ~ "Respondent and spouse/partner are the same age",
                     AGEPRGRD == 9  ~ "Respondent is 1 years younger",
                     AGEPRGRD == 10  ~ "Respondent is 2 years younger",
                     AGEPRGRD == 11  ~ "Respondent is 3 years younger",
                     AGEPRGRD == 12  ~ "Respondent is 4 years younger",
                     AGEPRGRD == 13  ~ "Respondent is 5 years younger",
                     AGEPRGRD == 14  ~ "Respondent is 6 - 10 years younger",
                     AGEPRGRD == 15  ~ "Respondent is 11 + years younger",
                     AGEPRGRD == 96 ~ "Valid skip",
                     AGEPRGRD == 97 ~ "Don't know",
                     AGEPRGRD == 98 ~ "Refusal",
                     AGEPRGRD == 99 ~ "Not stated"), 
    AGEPRGRD = factor(AGEPRGRD, levels = rev(values_ordered), ordered = TRUE)) 

summary(gss_main_2015$AGEPRGRD)
```



```{r SEXPR-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # SEXPR: Sex of respondent's spouse/partner living in the household.
    SEXPR = 
           case_when(SEXPR == 1 ~ "Male spouse/partner in the household", 
                     SEXPR == 2 ~ "Female spouse/partner in the household",
                     SEXPR == 6 ~ "Valid skip",
                     SEXPR == 7 ~ "Don't know",
                     SEXPR == 8 ~ "Refusal",
                     SEXPR == 9 ~ "Not stated"), 
    SEXPR = factor(SEXPR)) 

summary(gss_main_2015$SEXPR)
```

```{r PRTYPEC-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # PRTYPEC: Type of partner respondent has living in the household.
    PRTYPEC = 
           case_when(PRTYPEC == 1 ~ "Opposite sex married partner in the household", 
                     PRTYPEC == 2 ~ "Opposite sex common-law partner in the household",
                     PRTYPEC == 3 ~ "Same sex married or common-law partner in the household",
                     PRTYPEC == 6 ~ "Valid skip",
                     PRTYPEC == 7 ~ "Don't know",
                     PRTYPEC == 8 ~ "Refusal",
                     PRTYPEC == 9 ~ "Not stated"), 
    PRTYPEC = factor(PRTYPEC)) 

summary(gss_main_2015$PRTYPEC)
```


```{r CXRFLAG-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # CXRFLAG:  Child(ren) of the respondent living in the household.
    CXRFLAG = 
           case_when(CXRFLAG == 1 ~ "Yes", 
                     CXRFLAG == 2 ~ "No",
                     CXRFLAG == 6 ~ "Valid skip",
                     CXRFLAG == 7 ~ "Don't know",
                     CXRFLAG == 8 ~ "Refusal",
                     CXRFLAG == 9 ~ "Not stated"), 
    CXRFLAG = factor(CXRFLAG)) 

summary(gss_main_2015$CXRFLAG)
```

```{r AGECXRYG-2015}
values_ordered <- c( "Valid skip", "5 to 4", "5 to 9","10 to 14",
                     "15 to 19","20 to 24",
                     "25 years and older")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # AGECXRYG: Age of respondent's youngest child in household
    AGECXRYG = 
           case_when(AGECXRYG == 1 ~ "5 to 4", 
                     AGECXRYG == 2 ~ "5 to 9",
                     AGECXRYG == 3 ~ "10 to 14",
                     AGECXRYG == 4 ~ "15 to 19",
                     AGECXRYG == 5 ~ "20 to 24",
                     AGECXRYG == 6  ~ "25 years and older",
                     AGECXRYG == 96 ~ "Valid skip",
                     AGECXRYG == 97 ~ "Don't know",
                     AGECXRYG == 98 ~ "Refusal",
                     AGECXRYG == 99 ~ "Not stated"), 
    AGECXRYG = factor(AGECXRYG, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2015$AGECXRYG)
```

```{r CHINHSDC-2015}
values_ordered <- c("None", 
                     "One",
                     "Two",
                     "Three",
                     "Four or more")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # CHINHSDC: Respondent’s child(ren) in household - Any age/marital status
    CHINHSDC = 
           case_when(CHINHSDC == 0 ~ "None", 
                     CHINHSDC == 1 ~ "One",
                     CHINHSDC == 2 ~ "Two",
                     CHINHSDC == 3 ~ "Three",
                     CHINHSDC == 4 ~ "Four or more",
                     CHINHSDC == 6 ~ "Valid skip",
                     CHINHSDC == 7 ~ "Don't know",
                     CHINHSDC == 8 ~ "Refusal",
                     CHINHSDC == 9 ~ "Not stated"), 
    CHINHSDC = factor(CHINHSDC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2015$CHINHSDC)
```

```{r CXR0014C-2015}
values_ordered <- c("None", 
                     "One",
                     "Two",
                     "Three or more")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # CXR0014C:  Respondent's child(ren) in household - 0 to 14 years
    CXR0014C = 
           case_when(CXR0014C == 0 ~ "None", 
                     CXR0014C == 1 ~ "One",
                     CXR0014C == 2 ~ "Two",
                     CXR0014C == 3 ~ "Three or more",
                     CXR0014C == 6 ~ "Valid skip",
                     CXR0014C == 7 ~ "Don't know",
                     CXR0014C == 8 ~ "Refusal",
                     CXR0014C == 9 ~ "Not stated"), 
    CXR0014C = factor(CXR0014C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2015$CXR0014C)
```

```{r CHH0014C-2015}
values_ordered <- c("None", 
                     "One",
                     "Two",
                     "Three or more")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # CHH0014C:  Child(ren) in household - 0 to 14 years
    CHH0014C = 
           case_when(CHH0014C == 0 ~ "None", 
                     CHH0014C == 1 ~ "One",
                     CHH0014C == 2 ~ "Two",
                     CHH0014C == 3 ~ "Three or more",
                     CHH0014C == 6 ~ "Valid skip",
                     CHH0014C == 7 ~ "Don't know",
                     CHH0014C == 8 ~ "Refusal",
                     CHH0014C == 9 ~ "Not stated"), 
    CHH0014C = factor(CHH0014C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2015$CHH0014C)
```


```{r CXRTIME6-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # CXRTIME6:  Age group of respondent’s child(ren) in household
    CXRTIME6 = 
           case_when(CXRTIME6 == 1 ~ "No child under 19 years of age at home",
                     CXRTIME6 == 2 ~ "All children under 5 years of age",
                     CXRTIME6 == 3 ~ "All children between 5 and 12 years of age",
                     CXRTIME6 == 4 ~ "All children 13 years of age and older",
                     CXRTIME6 == 5 ~ "At least one child under 5 years of age but not all
children",
                     CXRTIME6 == 6 ~ "Other",
                     CXRTIME6 == 96 ~ "Valid skip",
                     CXRTIME6 == 97 ~ "Don't know",
                     CXRTIME6 == 98 ~ "Refusal",
                     CXRTIME6 == 99 ~ "Not stated"), 
    CXRTIME6 = factor(CXRTIME6))

summary(gss_main_2015$CXRTIME6)
```

```{r PARNUM-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # PARNUM: Number of parents the respondent has in household
    PARNUM = 
           case_when(PARNUM == 0 ~ "None",
                     PARNUM == 1 ~ "One parent",
                     PARNUM == 2 ~ "Two parents",
                     PARNUM == 6 ~ "Valid skip",
                     PARNUM == 7 ~ "Don't know",
                     PARNUM == 8 ~ "Refusal",
                     PARNUM == 9 ~ "Not stated"), 
    PARNUM = factor(PARNUM)) 

summary(gss_main_2015$PARNUM)
```

```{r LIVARR08-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # LIVARR08: Living arrangement of respondent’s household (8 categories)
    LIVARR08 = 
           case_when(LIVARR08 == 1 ~ "Under 25 years - Not living with parents",
                     LIVARR08 == 2 ~ "Under 25 years - Living with both parents",
                     LIVARR08 == 3 ~ "Under 25 years - Living with mother only",
                     LIVARR08 == 4 ~ "Under 25 years - Living with father only",
                     LIVARR08 == 5 ~ "25 years of age or older - Not living with parents",
                     LIVARR08 == 6 ~ "25 years of age or older - Living with both parents",
                     LIVARR08 == 7 ~ "25 years of age or older - Living with mother only",
                     LIVARR08 == 8 ~ "25 years of age or older - Living with father only",
                     LIVARR08 == 96 ~ "Valid skip",
                     LIVARR08 == 97 ~ "Don't know",
                     LIVARR08 == 98 ~ "Refusal",
                     LIVARR08 == 99 ~ "Not stated"), 
    LIVARR08 = factor(LIVARR08)) 

summary(gss_main_2015$LIVARR08)
```

```{r LIVARR11-2015} 
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # LIVARR11: Living arrangement of respondent’s household (11 categories)
    LIVARR11 = 
           case_when(LIVARR11 == 1 ~ "Living alone",
                     LIVARR11 == 2 ~ "Living with sponse only",
                     LIVARR11 == 3 ~ "Living with spouse and child(ren) < 25 years",
                     LIVARR11 == 4 ~ "Living with spouse and child(ren) 25 years or older",
                     LIVARR11 == 5 ~ "Living with spouse and other",
                     LIVARR11 == 6 ~ "Living without spouse but with chil(ren) < 25 years",
                     LIVARR11 == 7 ~ "Living without spouse but with chil(ren) 25 years and older",
                     LIVARR11 == 8 ~ "Living with two parents",
                     LIVARR11 == 9 ~ "Living with one parent",
                     LIVARR11 == 10 ~ "Multiple person household - Other living arrangement",
                     LIVARR11 == 11 ~ "Multiple person household - Relationships undefined",
                     LIVARR11 == 96 ~ "Valid skip",
                     LIVARR11 == 97 ~ "Don't know",
                     LIVARR11 == 98 ~ "Refusal",
                     LIVARR11 == 99 ~ "Not stated"), 
    LIVARR11 = factor(LIVARR11)) 

summary(gss_main_2015$LIVARR11)
```


```{r HSDSIZEC-2015}
values_ordered <- c("One person household",
                   "Two person household",
                   "Three person household",
                   "Four person household",
                   "Five person household",
                   "Six or more person household")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # HSDSIZEC: Household size of respondent
    HSDSIZEC = 
           case_when(HSDSIZEC == 1 ~ "One person household",
                     HSDSIZEC == 2 ~ "Two person household",
                     HSDSIZEC == 3 ~ "Three person household",
                     HSDSIZEC == 4 ~ "Four person household",
                     HSDSIZEC == 5 ~ "Five person household",
                     HSDSIZEC == 6 ~ "Six or more person household",
                     HSDSIZEC == 96 ~ "Valid skip",
                     HSDSIZEC == 97 ~ "Don't know",
                     HSDSIZEC == 98 ~ "Refusal",
                     HSDSIZEC == 99 ~ "Not stated"), 
    HSDSIZEC = factor(HSDSIZEC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2015$HSDSIZEC)
```

```{r HSDELIGC-2015}
values_ordered <- c("One member",
                     "Two members",
                     "Three members",
                     "Four or more members")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # HSDELIGC: Number of members in respondent’s household 15 years of age or older
    HSDELIGC = 
           case_when(HSDELIGC == 1 ~ "One member",
                     HSDELIGC == 2 ~ "Two members",
                     HSDELIGC == 3 ~ "Three members",
                     HSDELIGC == 4 ~ "Four or more members",
                     HSDELIGC == 6 ~ "Valid skip",
                     HSDELIGC == 7 ~ "Don't know",
                     HSDELIGC == 8 ~ "Refusal",
                     HSDELIGC == 9 ~ "Not stated"), 
    HSDELIGC = factor(HSDELIGC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2015$HSDELIGC)
```

```{r MULTIGEN-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # MULTIGEN:  Three generations or more in the respondent’s household
    MULTIGEN = 
           case_when(MULTIGEN == 1 ~ "Yes", 
                     MULTIGEN == 2 ~ "No",
                     MULTIGEN == 6 ~ "Valid skip",
                     MULTIGEN == 7 ~ "Don't know",
                     MULTIGEN == 8 ~ "Refusal",
                     MULTIGEN == 9 ~ "Not stated"), 
    MULTIGEN = factor(MULTIGEN))

summary(gss_main_2015$MULTIGEN)
```


```{r PRV-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # PRV: Province of residence
    PRV = 
      case_when(PRV == 10 ~ "Newfoundland and Labrador",
                     PRV == 11 ~ "Prince Edward Island",
                     PRV == 12 ~ "Nova Scotia",
                     PRV == 13 ~ "New Brunswick",
                     PRV == 24 ~ "Quebec",
                     PRV == 35 ~ "Ontario",
                     PRV == 46 ~ "Manitoba",
                     PRV == 47 ~ "Saskatchewan",
                     PRV == 48 ~ "Alberta",
                     PRV == 59 ~ "British Columbia",
                     PRV == 96 ~ "Valid skip",
                     PRV == 97 ~ "Don't know",
                     PRV == 98 ~ "Refusal",
                     PRV == 99 ~ "Not stated"),
    PRV = factor(PRV)) 

summary(gss_main_2015$PRV) 
```

```{r LUC_RST-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # LUC_RST:  Population centre indicator
    LUC_RST = 
      case_when(LUC_RST == 1 ~ "CMA/CA",
                     LUC_RST == 2 ~ "non CMA/CA",
                     LUC_RST == 3 ~ "Prince Edward Island",
                     LUC_RST == 6 ~ "Valid skip",
                     LUC_RST == 7 ~ "Don't know",
                     LUC_RST == 8 ~ "Refusal",
                     LUC_RST == 9 ~ "Not stated"),
    LUC_RST = factor(LUC_RST)) 

summary(gss_main_2015$LUC_RST)
```


```{r GTU_110-2015}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never", 
                    "Don't know")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # GTU_110: How often do you feel rushed? Would you say it is...?
    GTU_110 = 
      case_when(GTU_110 == 1 ~ "Every day",
                     GTU_110 == 2 ~ "A few times a week",
                     GTU_110 == 3 ~ "About once a week",
                     GTU_110 == 4 ~ "About once a month",
                     GTU_110 == 5 ~ "Less than once a month",
                     GTU_110 == 6 ~ "Never",
                     GTU_110 == 96 ~ "Valid skip",
                     GTU_110 == 97 ~ "Don't know",
                     GTU_110 == 98 ~ "Refusal",
                     GTU_110 == 99 ~ "Not stated"),
    GTU_110 = factor(GTU_110, levels = rev(values_ordered), ordered = TRUE)) 

summary(gss_main_2015$GTU_110)
```

```{r GTU_130-2015}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never", 
                    "Don't know")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # GTU_130: How often do you feel you have time on your hands that you don’t know what to do with?
    GTU_130 = 
      case_when(GTU_130 == 1 ~ "Every day",
                     GTU_130 == 2 ~ "A few times a week",
                     GTU_130 == 3 ~ "About once a week",
                     GTU_130 == 4 ~ "About once a month",
                     GTU_130 == 5 ~ "Less than once a month",
                     GTU_130 == 6 ~ "Never",
                     GTU_130 == 96 ~ "Valid skip",
                     GTU_130 == 97 ~ "Don't know",
                     GTU_130 == 98 ~ "Refusal",
                     GTU_130 == 99 ~ "Not stated"),
    GTU_130 = factor(GTU_130, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_2015$GTU_130)
```

```{r SLM_01-2015}
values_ordered <- c("10 - Very satisfied","9","8","7","6","5",
                    "4","3","2","1","0 - Very dissatisfied",
                    "Valid skip", 
                    "Don’t know", 
                    "Refusal", 
                    "Not stated")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # SLM_01: Using a scale of 0 to 10 where 0 means "Very dissatisfied" and 10 means "Very satisfied",how do you feel about your life as a whole right now? 
    SLM_01 = 
      case_when(SLM_01 == 0 ~ "0 - Very dissatisfied",
                SLM_01 == 1 ~ "1",
                     SLM_01 == 2 ~ "2",
                     SLM_01 == 3 ~ "3",
                     SLM_01 == 4 ~ "4",
                     SLM_01 == 5 ~ "5",
                     SLM_01 == 6 ~ "6",
                     SLM_01 == 7 ~ "7",
                     SLM_01 == 8 ~ "8",
                     SLM_01 == 9 ~ "9",
                     SLM_01 == 10 ~ "10 - Very satisfied",
                     SLM_01 == 96 ~ "Valid skip",
                     SLM_01 == 97 ~ "Don’t know",
                     SLM_01 == 98 ~ "Refusal",
                     SLM_01 == 99 ~ "Not stated"),
    SLM_01 = factor(SLM_01, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_2015$SLM_01)
```

```{r SRH_110-2015}
values_ordered <- c("Excellent","Very good","Good",
                    "Fair","Poor",
                    "Valid skip", 
                    "Don’t know", 
                    "Refusal", 
                    "Not stated")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # SRH_110: In general, would you say your health is...? 
    SRH_110 = 
      case_when(SRH_110 == 1 ~ "Excellent",
                     SRH_110 == 2 ~ "Very good",
                     SRH_110 == 3 ~ "Good",
                     SRH_110 == 4 ~ "Fair",
                     SRH_110 == 5 ~ "Poor",
                     SRH_110 == 6 ~ "Valid skip",
                     SRH_110 == 7 ~ "Don’t know",
                     SRH_110 == 8 ~ "Refusal",
                     SRH_110 == 9 ~ "Not stated"),
    SRH_110 = factor(SRH_110, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_2015$SRH_110)
```

```{r SRH_115-2015}
values_ordered <- c("Excellent","Very good","Good",
                    "Fair","Poor",
                    "Valid skip", 
                    "Don’t know", 
                    "Refusal", 
                    "Not stated")

gss_main_2015 <- gss_main_2015 %>% 
  mutate( # SRH_115: In general, would you say your mental health is...?
    SRH_115 = 
      case_when(SRH_115 == 1 ~ "Excellent",
                     SRH_115 == 2 ~ "Very good",
                     SRH_115 == 3 ~ "Good",
                     SRH_115 == 4 ~ "Fair",
                     SRH_115 == 5 ~ "Poor",
                     SRH_115 == 6 ~ "Valid skip",
                     SRH_115 == 7 ~ "Don’t know",
                     SRH_115 == 8 ~ "Refusal",
                     SRH_115 == 9 ~ "Not stated"),
    SRH_115 = factor(SRH_115, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_2015$SRH_115)
```

```{r DVTDAY-2015}
gss_main_2015 <- gss_main_2015 %>% 
  mutate( # DVTDAY: Diary - Reference day - Type
    DVTDAY = 
      case_when(DVTDAY == 1 ~ "Weekday",
                     DVTDAY == 2 ~ "Saturday",
                     DVTDAY == 3 ~ "Sunday",
                     DVTDAY == 96 ~ "Valid skip",
                     DVTDAY == 97 ~ "Don't know",
                     DVTDAY == 98 ~ "Refusal",
                     DVTDAY == 99 ~ "Not stated"),
    DVTDAY = factor(DVTDAY)) 

summary(gss_main_2015$DVTDAY)
```

Save data in R format for later use:
```{r save-data-2015}
usethis::use_data(gss_main_2015, 
                  overwrite = TRUE)
```

## GSS 2010

Read the Main files of the 2010 GSS survey:

```{r reading-2010-main-file}
# Main File
gss_m_2010 <- read.csv(paste0(here(), "/data-raw/source-files/Time-use-2010/gss-12M0018-E-2010-c-24-tus-mf_F1.csv"))
```

Select only the socioeconomic variables: 

```{r filtering-variables-2010}
gss_main_2010 <- gss_m_2010 %>%
  select(RECID:DVTDAY)
```

Labeling socioeconomic variables: 

```{r AGEGR5-2010}
values_ordered <- c("15 to 17 years","18 to 19 years","20 to 24 years",
                    "25 to 29 years","30 to 34 years","35 to 39 year","40 to 44 years",
                    "45 to 49 years","50 to 54 years","55 to 59 year","60 to 64 years",
                    "65 to 69 years","70 to 74 years","75 to 79 year","80 years and over")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # AGEGR5: Age group of respondent (groups of 5)
    AGEGR5 = 
           case_when(AGEGR5 == 1 ~ "15 to 17 years",
                     AGEGR5 == 2 ~ "18 to 19 years",
                     AGEGR5 == 3 ~ "20 to 24 years",
                     AGEGR5 == 4 ~ "25 to 29 years",
                     AGEGR5 == 5 ~ "30 to 34 years",
                     AGEGR5 == 6  ~ "35 to 39 year",
                     AGEGR5 == 7 ~ "40 to 44 years",
                     AGEGR5 == 8 ~ "45 to 49 years",
                     AGEGR5 == 9 ~ "50 to 54 years",
                     AGEGR5 == 10 ~ "55 to 59 year",
                     AGEGR5 == 11 ~ "60 to 64 years",
                     AGEGR5 == 12 ~ "65 to 69 years",
                     AGEGR5 == 13 ~ "70 to 74 years",
                     AGEGR5 == 14 ~ "75 to 79 year",
                     AGEGR5 == 15 ~ "80 years and over"), 
    AGEGR5 = factor(AGEGR5, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2010$AGEGR5)
```


```{r AGEGR10-2010}
values_ordered <- c("15 to 24 years","25 to 34 years","35 to 44 years",
                    "45 to 54 years", "55 to 64 years","65 to 74 years","75 years and over")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # AGEGR10: Age group of respondent (groups of 10)
    AGEGR10 = 
           case_when(AGEGR10 == 1 ~ "15 to 24 years", 
                     AGEGR10 == 2 ~ "25 to 34 years",
                     AGEGR10 == 3 ~ "35 to 44 years",
                     AGEGR10 == 4 ~ "45 to 54 years",
                     AGEGR10 == 5 ~ "55 to 64 years",
                     AGEGR10 == 6  ~ "65 to 74 years",
                     AGEGR10 == 7 ~ "75 years and over"), 
    AGEGR10 = factor(AGEGR10, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2010$AGEGR10)
```

```{r SEX-2010}
values_ordered <- c("Male", "Female")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # SEX: Sex of respondent
    SEX = 
           case_when(SEX == 1 ~ "Male", 
                     SEX == 2 ~ "Female"), 
    SEX = factor(SEX, levels = values_ordered, ordered = FALSE)) 

summary(gss_main_2010$SEX)
```

```{r MARSTAT-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # MARSTAT: Marital status of the respondent
    MARSTAT = 
           case_when(MARSTAT == 1 ~ "Married", 
                     MARSTAT == 2 ~ "Living common-law",
                     MARSTAT == 3 ~ "Widowed",
                     MARSTAT == 4 ~ "Separated",
                     MARSTAT == 5 ~ "Divorced",
                     MARSTAT == 6  ~ "Single, never married",
                     MARSTAT == 8 ~ "Don't know",
                     MARSTAT == 9 ~ "Not stated"), 
    MARSTAT = factor(MARSTAT))

summary(gss_main_2010$MARSTAT)
```

```{r AGEPRGRDIF-2010}
values_ordered <- c("Respondent is 11 + years older", 
                    "Respondent is 6 - 10 years older",
                    "Respondent is 5 years older",
                    "Respondent is 4 years older",
                    "Respondent is 3 years older",
                    "Respondent is 2 years older",
                    "Respondent is 1 years older",
                    "Respondent and spouse/partner are the same age",
                    "Respondent is 1 years younger",
                    "Respondent is 2 years younger",
                    "Respondent is 3 years younger",
                    "Respondent is 4 years younger",
                    "Respondent is 5 years younger",
                    "Respondent is 6 - 10 years younger",
                    "Respondent is 11 + years younger",
                    "No spouse/partner in household")
                    
                    
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # AGEPRGRDIF: Age difference between respondent and spouse/partner.
    AGEPRGRDIF = 
           case_when(AGEPRGRDIF == 1 ~ "Respondent is 11 + years older", 
                     AGEPRGRDIF == 2 ~ "Respondent is 6 - 10 years older",
                     AGEPRGRDIF == 3 ~ "Respondent is 5 years older",
                     AGEPRGRDIF == 4 ~ "Respondent is 4 years older",
                     AGEPRGRDIF == 5 ~ "Respondent is 3 years older",
                     AGEPRGRDIF == 6  ~ "Respondent is 2 years older",
                     AGEPRGRDIF == 7  ~ "Respondent is 1 years older",
                     AGEPRGRDIF == 8  ~ "Respondent and spouse/partner are the same age",
                     AGEPRGRDIF == 9  ~ "Respondent is 1 years younger",
                     AGEPRGRDIF == 10  ~ "Respondent is 2 years younger",
                     AGEPRGRDIF == 11  ~ "Respondent is 3 years younger",
                     AGEPRGRDIF == 12  ~ "Respondent is 4 years younger",
                     AGEPRGRDIF == 13  ~ "Respondent is 5 years younger",
                     AGEPRGRDIF == 14  ~ "Respondent is 6 - 10 years younger",
                     AGEPRGRDIF == 15  ~ "Respondent is 11 + years younger",
                     AGEPRGRDIF == 97 ~ "No spouse/partner in household"), 
    AGEPRGRDIF = factor(AGEPRGRDIF, levels = rev(values_ordered), ordered = TRUE)) 

summary(gss_main_2010$AGEPRGRDIF)
```


```{r SEXPR-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # SEXPR: Sex of respondent's spouse/partner living in the household.
    SEXPR = 
           case_when(SEXPR == 1 ~ "Male spouse/partner in the household", 
                     SEXPR == 2 ~ "Female spouse/partner in the household",
                     SEXPR == 0 ~ "Respondent has no partner in the household"), 
    SEXPR = factor(SEXPR)) 

summary(gss_main_2010$SEXPR)
```

```{r PRTYPEC-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # PRTYPEC: Type of partner respondent has living in the household.
    PRTYPEC = 
           case_when(PRTYPEC == 1 ~ "Opposite sex married partner in the household", 
                     PRTYPEC == 2 ~ "Opposite sex common-law partner in the household",
                     PRTYPEC == 3 ~ "Same sex married or common-law partner in the household",
                     PRTYPEC == 0 ~ "Respondent has no spouse/partner in the household "), 
    PRTYPEC = factor(PRTYPEC)) 

summary(gss_main_2010$PRTYPEC)
```

```{r AGECHRYC-2010}

values_ordered <- c(as.character(0:24),"25 and over","Not asked - no children in the household")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # AGECHRYC: Age of respondent's youngest child in household
           AGECHRYC = case_when(AGECHRYC == 25 ~ "25 and over",
                     AGECHRYC == 97 ~ "Not asked - no children in the household",
                     TRUE ~ as.character(AGECHRYC)), 
    AGECHRYC = factor(AGECHRYC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2010$AGECHRYC)
```

```{r CHRFLAG-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # CHRFLAG:  single child(ren) of the respondent living in the household.
    CHRFLAG = 
           case_when(CHRFLAG == 1 ~ "Yes", 
                     CHRFLAG == 2 ~ "No"), 
    CHRFLAG = factor(CHRFLAG)) 

summary(gss_main_2010$CHRFLAG)
```

```{r CHRINHSDC-2010}
values_ordered <- c("None", "One",
                     "Two",
                     "Three",
                     "Four or more children")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # CHRINHSDC: Number of respondent's children living in the household (any age or marital status).
    CHRINHSDC = 
           case_when(CHRINHSDC == 0 ~ "None",
                     CHRINHSDC == 1 ~ "One",
                     CHRINHSDC == 2 ~ "Two",
                     CHRINHSDC == 3 ~ "Three",
                     CHRINHSDC == 4 ~ "Four or more children"), 
    CHRINHSDC = factor(CHRINHSDC, levels = values_ordered, ordered = TRUE)) 
summary(gss_main_2010$CHRINHSDC)
```

```{r CHR0014C-2010}
values_ordered <- c("None", "One",
                     "Two",
                     "Three or more children")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # CHR0014C: Number of respondent's children living in the household (any age or marital status).
    CHR0014C = 
           case_when(CHR0014C == 0 ~ "None",
                     CHR0014C == 1 ~ "One",
                     CHR0014C == 2 ~ "Two",
                     CHR0014C == 3 ~ "Three or more children"), 
    CHR0014C = factor(CHR0014C, levels = values_ordered, ordered = TRUE)) 
summary(gss_main_2010$CHR0014C)
```

```{r CHRTIME6-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # CHRTIME6:  Age group of respondent’s child(ren) in household
    CHRTIME6 = 
           case_when(CHRTIME6 == 1 ~ "No child under 19 years of age at home",
                     CHRTIME6 == 2 ~ "All children under 5 years of age",
                     CHRTIME6 == 3 ~ "All children between 5 and 12 years of age",
                     CHRTIME6 == 4 ~ "All children 13 years of age and older",
                     CHRTIME6 == 5 ~ "At least one child under 5 years of age but not all children",
                     CHRTIME6 == 6 ~ "Other"), 
    CHRTIME6 = factor(CHRTIME6))

summary(gss_main_2010$CHRTIME6)
```

```{r CHH0512C-2010}
values_ordered <- c("None", "One",
                     "Two",
                     "Three or more children")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # CHH0512C: Number of children aged from 5 to 12 years living in the respondent's household.
    CHH0512C = 
           case_when(CHH0512C == 0 ~ "None",
                     CHH0512C == 1 ~ "One",
                     CHH0512C == 2 ~ "Two",
                     CHH0512C == 3 ~ "Three or more children"), 
    CHH0512C = factor(CHH0512C, levels = values_ordered, ordered = TRUE)) 
summary(gss_main_2010$CHH0512C)
```


```{r CHH0014C-2010}
values_ordered <- c("None", "One",
                     "Two",
                     "Three or more children")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # CHH0014C: Number of children aged from 0 to 14 years living in the respondent's household.
    CHH0014C = 
           case_when(CHH0014C == 0 ~ "None",
                     CHH0014C == 1 ~ "One",
                     CHH0014C == 2 ~ "Two",
                     CHH0014C == 3 ~ "Three or more children"), 
    CHH0014C = factor(CHH0014C, levels = values_ordered, ordered = TRUE)) 
summary(gss_main_2010$CHH0014C)
```

```{r PARHSDC-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # PARHSDC: Type of parents the respondent has within the household.
    PARHSDC = 
           case_when(PARHSDC == 1 ~ "Two biological parents",
                     PARHSDC == 2 ~ "Two parents (biological/adoptive/step)",
                     PARHSDC == 3 ~ "Only one parent",
                     PARHSDC == 7 ~ "Not asked - no parents in the household"), 
    PARHSDC = factor(PARHSDC)) 

summary(gss_main_2010$PARHSDC)
```

```{r LIVARR08-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # LIVARR08: Living arrangement of respondent’s household (8 categories)
    LIVARR08 = 
           case_when(LIVARR08 == 1 ~ "Under 25 years - Not living with parents",
                     LIVARR08 == 2 ~ "Under 25 years - Living with both parents",
                     LIVARR08 == 3 ~ "Under 25 years - Living with mother only",
                     LIVARR08 == 4 ~ "Under 25 years - Living with father only",
                     LIVARR08 == 5 ~ "25 years of age or older - Not living with parents",
                     LIVARR08 == 6 ~ "25 years of age or older - Living with both parents",
                     LIVARR08 == 7 ~ "25 years of age or older - Living with mother only",
                     LIVARR08 == 8 ~ "25 years of age or older - Living with father only"), 
    LIVARR08 = factor(LIVARR08)) 

summary(gss_main_2010$LIVARR08)
```

```{r LIVARR12-2010} 
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # LIVARR12: Living arrangement of respondent’s household (12 categories)
    LIVARR12 = 
           case_when(LIVARR12 == 1 ~ "Living alone",
                     LIVARR12 == 2 ~ "Living with sponse only",
                     LIVARR12 == 3 ~ "Living with spouse and single child < 25 years",
                     LIVARR12 == 4 ~ "Living with spouse and single child 25 years or older",
                     LIVARR12 == 5 ~ "Living with spouse and non-child(ren)",
                     LIVARR12 == 6 ~ "Living with spouse and other",
                     LIVARR12 == 7 ~ "Living without spouse and single child  < 25 years",
                     LIVARR12 == 8 ~ "Living without spouse and single child 25 years or older",
                     LIVARR12 == 9 ~ "No spouse and non-single child(ren)",
                     LIVARR12 == 10 ~ "Living with two parents",
                     LIVARR12 == 11 ~ "Living with one parent",
                     LIVARR12 == 12 ~ "Other living arrangement"), 
    LIVARR12 = factor(LIVARR12)) 

summary(gss_main_2010$LIVARR12)
```

```{r HSDSIZEC-2010}
values_ordered <- c("One person household",
                   "Two person household",
                   "Three person household",
                   "Four person household",
                   "Five person household",
                   "Six or more person household")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # HSDSIZEC: Household size of respondent
    HSDSIZEC = 
           case_when(HSDSIZEC == 1 ~ "One person household",
                     HSDSIZEC == 2 ~ "Two person household",
                     HSDSIZEC == 3 ~ "Three person household",
                     HSDSIZEC == 4 ~ "Four person household",
                     HSDSIZEC == 5 ~ "Five person household",
                     HSDSIZEC == 6 ~ "Six or more person household"), 
    HSDSIZEC = factor(HSDSIZEC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2010$HSDSIZEC)
```

```{r HSDELIGC-2010}
values_ordered <- c("One member",
                     "Two members",
                     "Three members",
                     "Four members", "Five household members or more")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # HSDELIGC: Number of members in respondent’s household 15 years of age or older
    HSDELIGC = 
           case_when(HSDELIGC == 1 ~ "One member",
                     HSDELIGC == 2 ~ "Two members",
                     HSDELIGC == 3 ~ "Three members",
                     HSDELIGC == 4 ~ "Four members",
                     HSDELIGC == 5 ~ "Five household members or more"), 
    HSDELIGC = factor(HSDELIGC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2010$HSDELIGC)
```

```{r FAMTYPE-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # FAMTYPE: Respondent's type of family structure.
    FAMTYPE = 
           case_when(FAMTYPE == 1 ~ "Couple only", 
                     FAMTYPE == 2 ~ "Intact family",
                     FAMTYPE == 3 ~ "Step family with common child",
                     FAMTYPE == 4 ~ "Step family without a common child",
                     FAMTYPE == 5 ~ "Lone parent family",
                     FAMTYPE == 7 ~ "No spouse/partner or children in household"), 
    FAMTYPE = factor(FAMTYPE))

summary(gss_main_2010$FAMTYPE)
```

```{r MULTIGEN-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # MULTIGEN:  Three generations or more in the respondent’s household
    MULTIGEN = 
           case_when(MULTIGEN == 1 ~ "Yes", 
                     MULTIGEN == 2 ~ "No",
                     MULTIGEN == 6 ~ "Number of generations unknown"), 
    MULTIGEN = factor(MULTIGEN))

summary(gss_main_2010$MULTIGEN)
```


```{r PRV-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # PRV: Province of residence
    PRV = 
      case_when(PRV == 10 ~ "Newfoundland and Labrador",
                     PRV == 11 ~ "Prince Edward Island",
                     PRV == 12 ~ "Nova Scotia",
                     PRV == 13 ~ "New Brunswick",
                     PRV == 24 ~ "Quebec",
                     PRV == 35 ~ "Ontario",
                     PRV == 46 ~ "Manitoba",
                     PRV == 47 ~ "Saskatchewan",
                     PRV == 48 ~ "Alberta",
                     PRV == 59 ~ "British Columbia"),
    PRV = factor(PRV)) 

summary(gss_main_2010$PRV) 
```

```{r REGION-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # REGION:  Region of residence of the respondent.
    REGION = 
      case_when(REGION == 1 ~ "Atlantic region",
                     REGION == 2 ~ "Quebec",
                     REGION == 3 ~ "Ontario",
                     REGION == 4 ~ "Prairie region",
                     REGION == 5 ~ "British Columbia"),
    REGION = factor(REGION)) 

summary(gss_main_2010$REGION)
```

```{r LUC_RST-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # LUC_RST:  Population centre indicator
    LUC_RST = 
      case_when(LUC_RST == 1 ~ "CMA/CA",
                     LUC_RST == 2 ~ "non CMA/CA",
                     LUC_RST == 3 ~ "Prince Edward Island"),
    LUC_RST = factor(LUC_RST)) 

summary(gss_main_2010$LUC_RST)
```


```{r GTU_110-2010}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never", 
                    "Don't know")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # GTU_Q110: How often do you feel rushed? Would you say it is...?
    GTU_Q110 = 
      case_when(GTU_Q110 == 1 ~ "Every day",
                     GTU_Q110 == 2 ~ "A few times a week",
                     GTU_Q110 == 3 ~ "About once a week",
                     GTU_Q110 == 4 ~ "About once a month",
                     GTU_Q110 == 5 ~ "Less than once a month",
                     GTU_Q110 == 6 ~ "Never",
                     GTU_Q110 == 9 ~ "Don't know"),
    GTU_Q110 = factor(GTU_Q110, levels = rev(values_ordered), ordered = TRUE)) 

summary(gss_main_2010$GTU_Q110)
```

```{r GTU_130-2010}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never", 
                    "Don't know")

gss_main_2010 <- gss_main_2010 %>% 
  mutate( # GTU_Q130: How often do you feel you have time on your hands that you don’t know what to do with?
    GTU_Q130 = 
      case_when(GTU_Q130 == 1 ~ "Every day",
                     GTU_Q130 == 2 ~ "A few times a week",
                     GTU_Q130 == 3 ~ "About once a week",
                     GTU_Q130 == 4 ~ "About once a month",
                     GTU_Q130 == 5 ~ "Less than once a month",
                     GTU_Q130 == 6 ~ "Never",
                     GTU_Q130 == 9 ~ "Don't know"),
    GTU_Q130 = factor(GTU_Q130, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_2010$GTU_Q130)
```

```{r DVTDAY-2010}
gss_main_2010 <- gss_main_2010 %>% 
  mutate( # DVTDAY: Diary - Reference day - Type
    DVTDAY = 
      case_when(DVTDAY == 1 ~ "Weekday",
                     DVTDAY == 2 ~ "Saturday",
                     DVTDAY == 3 ~ "Sunday"),
    DVTDAY = factor(DVTDAY)) 

summary(gss_main_2010$DVTDAY)
```

Save data in R format for later use:
```{r save-data-2010}
usethis::use_data(gss_main_2010, 
                  overwrite = TRUE)
```


## GSS 2005

Read the Main files of the 2005 GSS survey:

```{r reading-2005-main-file}
# Main File
gss_m_2005 <- read.csv(paste0(here(), "/data-raw/source-files/Time-use-2005/gss-12M0019-E-2005-c-19-m_F1.csv"))
```

Select only the socioeconomic variables: 

```{r filtering-variables-22005015}
gss_main_2005 <- gss_m_2005 %>%
  select(RECID:DVTDAY)
```

Labeling socioeconomic variables: 

```{r AGEGR5-2005}
values_ordered <- c("15 to 17 years","18 to 19 years","20 to 24 years",
                    "25 to 29 years","30 to 34 years","35 to 39 year","40 to 44 years",
                    "45 to 49 years","50 to 54 years","55 to 59 year","60 to 64 years",
                    "65 to 69 years","70 to 74 years","75 to 79 year","80 years and over")

gss_main_2005 <- gss_main_2005 %>% 
  mutate( # AGEGR5: Age group of respondent (groups of 5)
    AGEGR5 = 
           case_when(AGEGR5 == 1 ~ "15 to 17 years",
                     AGEGR5 == 2 ~ "18 to 19 years",
                     AGEGR5 == 3 ~ "20 to 24 years",
                     AGEGR5 == 4 ~ "25 to 29 years",
                     AGEGR5 == 5 ~ "30 to 34 years",
                     AGEGR5 == 6  ~ "35 to 39 year",
                     AGEGR5 == 7 ~ "40 to 44 years",
                     AGEGR5 == 8 ~ "45 to 49 years",
                     AGEGR5 == 9 ~ "50 to 54 years",
                     AGEGR5 == 10 ~ "55 to 59 year",
                     AGEGR5 == 11 ~ "60 to 64 years",
                     AGEGR5 == 12 ~ "65 to 69 years",
                     AGEGR5 == 13 ~ "70 to 74 years",
                     AGEGR5 == 14 ~ "75 to 79 year",
                     AGEGR5 == 15 ~ "80 years and over"), 
    AGEGR5 = factor(AGEGR5, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2005$AGEGR5)
```


```{r AGEGR10-2005}
values_ordered <- c("15 to 24 years","25 to 34 years","35 to 44 years",
                    "45 to 54 years", "55 to 64 years","65 to 74 years","75 years and over")

gss_main_2005 <- gss_main_2005 %>% 
  mutate( # AGEGR10: Age group of respondent (groups of 10)
    AGEGR10 = 
           case_when(AGEGR10 == 1 ~ "15 to 24 years", 
                     AGEGR10 == 2 ~ "25 to 34 years",
                     AGEGR10 == 3 ~ "35 to 44 years",
                     AGEGR10 == 4 ~ "45 to 54 years",
                     AGEGR10 == 5 ~ "55 to 64 years",
                     AGEGR10 == 6  ~ "65 to 74 years",
                     AGEGR10 == 7 ~ "75 years and over"), 
    AGEGR10 = factor(AGEGR10, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2005$AGEGR10)
```

```{r SEX-2005}
values_ordered <- c("Male", "Female")

gss_main_2005 <- gss_main_2005 %>% 
  mutate( # SEX: Sex of respondent
    SEX = 
           case_when(SEX == 1 ~ "Male", 
                     SEX == 2 ~ "Female"), 
    SEX = factor(SEX, levels = values_ordered, ordered = FALSE)) 

summary(gss_main_2005$SEX)
```

```{r MARSTAT-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # MARSTAT: Marital status of the respondent
    MARSTAT = 
           case_when(MARSTAT == 1 ~ "Married", 
                     MARSTAT == 2 ~ "Living common-law",
                     MARSTAT == 3 ~ "Widowed",
                     MARSTAT == 4 ~ "Separated",
                     MARSTAT == 5 ~ "Divorced",
                     MARSTAT == 6  ~ "Single, never married",
                     MARSTAT == 8 ~ "Don't know",
                     MARSTAT == 9 ~ "Not stated"), 
    MARSTAT = factor(MARSTAT))

summary(gss_main_2005$MARSTAT)
```

```{r AGEPRGRDIF-2005}
values_ordered <- c("Respondent is 11 + years older", 
                    "Respondent is 6 - 10 years older",
                    "Respondent is 5 years older",
                    "Respondent is 4 years older",
                    "Respondent is 3 years older",
                    "Respondent is 2 years older",
                    "Respondent is 1 years older",
                    "Respondent and spouse/partner are the same age",
                    "Respondent is 1 years younger",
                    "Respondent is 2 years younger",
                    "Respondent is 3 years younger",
                    "Respondent is 4 years younger",
                    "Respondent is 5 years younger",
                    "Respondent is 6 - 10 years younger",
                    "Respondent is 11 + years younger",
                    "No spouse/partner in household")
                    
                    
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # AGEPRGRDIF: Age difference between respondent and spouse/partner.
    AGEPRGRDIF = 
           case_when(AGEPRGRDIF == 1 ~ "Respondent is 11 + years older", 
                     AGEPRGRDIF == 2 ~ "Respondent is 6 - 10 years older",
                     AGEPRGRDIF == 3 ~ "Respondent is 5 years older",
                     AGEPRGRDIF == 4 ~ "Respondent is 4 years older",
                     AGEPRGRDIF == 5 ~ "Respondent is 3 years older",
                     AGEPRGRDIF == 6  ~ "Respondent is 2 years older",
                     AGEPRGRDIF == 7  ~ "Respondent is 1 years older",
                     AGEPRGRDIF == 8  ~ "Respondent and spouse/partner are the same age",
                     AGEPRGRDIF == 9  ~ "Respondent is 1 years younger",
                     AGEPRGRDIF == 10  ~ "Respondent is 2 years younger",
                     AGEPRGRDIF == 11  ~ "Respondent is 3 years younger",
                     AGEPRGRDIF == 12  ~ "Respondent is 4 years younger",
                     AGEPRGRDIF == 13  ~ "Respondent is 5 years younger",
                     AGEPRGRDIF == 14  ~ "Respondent is 6 - 10 years younger",
                     AGEPRGRDIF == 15  ~ "Respondent is 11 + years younger",
                     AGEPRGRDIF == 97 ~ "No spouse/partner in household"), 
    AGEPRGRDIF = factor(AGEPRGRDIF, levels = rev(values_ordered), ordered = TRUE)) 

summary(gss_main_2005$AGEPRGRDIF)
```


```{r SEXPR-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # SEXPR: Sex of respondent's spouse/partner living in the household.
    SEXPR = 
           case_when(SEXPR == 1 ~ "Male spouse/partner in the household", 
                     SEXPR == 2 ~ "Female spouse/partner in the household",
                     SEXPR == 0 ~ "Respondent has no partner in the household"), 
    SEXPR = factor(SEXPR)) 

summary(gss_main_2005$SEXPR)
```

```{r PRTYPEC-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # PRTYPEC: Type of partner respondent has living in the household.
    PRTYPEC = 
           case_when(PRTYPEC == 1 ~ "Respondent has a married partner in the household", 
                     PRTYPEC == 2 ~ "Respondent has a common-law partner in the household",
                     PRTYPEC == 0 ~ "Respondent has no spouse/partner in the household"), 
    PRTYPEC = factor(PRTYPEC)) 

summary(gss_main_2005$PRTYPEC)
```


```{r AGECHRYC-2005}

values_ordered <- c(as.character(0:24),"25 and over","Not asked - no children in the household")

gss_main_2005 <- gss_main_2005 %>% 
  mutate( # AGECHRYC: Age of respondent's youngest child in household
           AGECHRYC = case_when(AGECHRYC == 25 ~ "25 and over",
                     AGECHRYC == 97 ~ "Not asked - no children in the household",
                     TRUE ~ as.character(AGECHRYC)), 
    AGECHRYC = factor(AGECHRYC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2005$AGECHRYC)
```

```{r CHRFLAG-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # CHRFLAG:  single child(ren) of the respondent living in the household.
    CHRFLAG = 
           case_when(CHRFLAG == 1 ~ "Yes", 
                     CHRFLAG == 2 ~ "No"), 
    CHRFLAG = factor(CHRFLAG)) 

summary(gss_main_2005$CHRFLAG)
```

```{r CHRINHSDC-2005}
values_ordered <- c("None", "One",
                     "Two",
                     "Three",
                     "Four or more children")

gss_main_2005 <- gss_main_2005 %>% 
  mutate( # CHRINHSDC: Number of respondent's children living in the household (any age or marital status).
    CHRINHSDC = 
           case_when(CHRINHSDC == 0 ~ "None",
                     CHRINHSDC == 1 ~ "One",
                     CHRINHSDC == 2 ~ "Two",
                     CHRINHSDC == 3 ~ "Three",
                     CHRINHSDC == 4 ~ "Four or more children"), 
    CHRINHSDC = factor(CHRINHSDC, levels = values_ordered, ordered = TRUE)) 
summary(gss_main_2005$CHRINHSDC)
```

```{r CHR0014C-2005}
values_ordered <- c("None", "One",
                     "Two",
                     "Three or more children")

gss_main_2005 <- gss_main_2005 %>% 
  mutate( # CHR0014C: Number of respondent's children living in the household (any age or marital status).
    CHR0014C = 
           case_when(CHR0014C == 0 ~ "None",
                     CHR0014C == 1 ~ "One",
                     CHR0014C == 2 ~ "Two",
                     CHR0014C == 3 ~ "Three or more children"), 
    CHR0014C = factor(CHR0014C, levels = values_ordered, ordered = TRUE)) 
summary(gss_main_2005$CHR0014C)
```

```{r CHRTIME6-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # CHRTIME6:  Age group of respondent’s child(ren) in household
    CHRTIME6 = 
           case_when(CHRTIME6 == 1 ~ "No child under 19 years of age at home",
                     CHRTIME6 == 2 ~ "All children under 5 years of age",
                     CHRTIME6 == 3 ~ "All children between 5 and 12 years of age",
                     CHRTIME6 == 4 ~ "All children 13 years of age and older",
                     CHRTIME6 == 5 ~ "At least one child under 5 years of age but not all children",
                     CHRTIME6 == 6 ~ "Other"), 
    CHRTIME6 = factor(CHRTIME6))

summary(gss_main_2005$CHRTIME6)
```

```{r PARHSDC-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # PARHSDC: Type of parents the respondent has within the household.
    PARHSDC = 
           case_when(PARHSDC == 1 ~ "Two biological parents",
                     PARHSDC == 2 ~ "Two parents (biological/adoptive/step)",
                     PARHSDC == 3 ~ "Only one parent",
                     PARHSDC == 7 ~ "Not asked - no parents in the household"), 
    PARHSDC = factor(PARHSDC)) 

summary(gss_main_2005$PARHSDC)
```

```{r LIVARR08-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # LIVARR08: Living arrangement of respondent’s household (8 categories)
    LIVARR08 = 
           case_when(LIVARR08 == 1 ~ "Under 25 years - Not living with parents",
                     LIVARR08 == 2 ~ "Under 25 years - Living with both parents",
                     LIVARR08 == 3 ~ "Under 25 years - Living with mother only",
                     LIVARR08 == 4 ~ "Under 25 years - Living with father only",
                     LIVARR08 == 5 ~ "25 years of age or older - Not living with parents",
                     LIVARR08 == 6 ~ "25 years of age or older - Living with both parents",
                     LIVARR08 == 7 ~ "25 years of age or older - Living with mother only",
                     LIVARR08 == 8 ~ "25 years of age or older - Living with father only"), 
    LIVARR08 = factor(LIVARR08)) 

summary(gss_main_2005$LIVARR08)
```

```{r LIVARR12-2005} 
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # LIVARR12: Living arrangement of respondent’s household (12 categories)
    LIVARR12 = 
           case_when(LIVARR12 == 1 ~ "Living alone",
                     LIVARR12 == 2 ~ "Living with sponse only",
                     LIVARR12 == 3 ~ "Living with spouse and single child < 25 years",
                     LIVARR12 == 4 ~ "Living with spouse and single child 25 years or older",
                     LIVARR12 == 5 ~ "Living with spouse and non-child(ren)",
                     LIVARR12 == 6 ~ "Living with spouse and other",
                     LIVARR12 == 7 ~ "Living without spouse and single child  < 25 years",
                     LIVARR12 == 8 ~ "Living without spouse and single child 25 years or older",
                     LIVARR12 == 9 ~ "No spouse and non-single child(ren)",
                     LIVARR12 == 10 ~ "Living with two parents",
                     LIVARR12 == 11 ~ "Living with one parent",
                     LIVARR12 == 12 ~ "Other living arrangement"), 
    LIVARR12 = factor(LIVARR12)) 

summary(gss_main_2005$LIVARR12)
```

```{r HSDSIZEC-2005}
values_ordered <- c("One person household",
                   "Two person household",
                   "Three person household",
                   "Four person household",
                   "Five person household",
                   "Six or more person household")

gss_main_2005 <- gss_main_2005 %>% 
  mutate( # HSDSIZEC: Household size of respondent
    HSDSIZEC = 
           case_when(HSDSIZEC == 1 ~ "One person household",
                     HSDSIZEC == 2 ~ "Two person household",
                     HSDSIZEC == 3 ~ "Three person household",
                     HSDSIZEC == 4 ~ "Four person household",
                     HSDSIZEC == 5 ~ "Five person household",
                     HSDSIZEC == 6 ~ "Six or more person household"), 
    HSDSIZEC = factor(HSDSIZEC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_2005$HSDSIZEC)
```

```{r FAMTYPE-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # FAMTYPE: Respondent's type of family structure.
    FAMTYPE = 
           case_when(FAMTYPE == 1 ~ "Couple only", 
                     FAMTYPE == 2 ~ "Intact family",
                     FAMTYPE == 3 ~ "Step family with common child",
                     FAMTYPE == 4 ~ "Step family without a common child",
                     FAMTYPE == 5 ~ "Lone parent family",
                     FAMTYPE == 7 ~ "No spouse/partner or children in household"), 
    FAMTYPE = factor(FAMTYPE))

summary(gss_main_2005$FAMTYPE)
```

```{r MULTIGEN-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # MULTIGEN:  Three generations or more in the respondent’s household
    MULTIGEN = 
           case_when(MULTIGEN == 1 ~ "Yes", 
                     MULTIGEN == 2 ~ "No",
                     MULTIGEN == 6 ~ "Number of generations unknown"), 
    MULTIGEN = factor(MULTIGEN))

summary(gss_main_2005$MULTIGEN)
```


```{r PRV-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # PRV: Province of residence
    PRV = 
      case_when(PRV == 10 ~ "Newfoundland and Labrador",
                     PRV == 11 ~ "Prince Edward Island",
                     PRV == 12 ~ "Nova Scotia",
                     PRV == 13 ~ "New Brunswick",
                     PRV == 24 ~ "Quebec",
                     PRV == 35 ~ "Ontario",
                     PRV == 46 ~ "Manitoba",
                     PRV == 47 ~ "Saskatchewan",
                     PRV == 48 ~ "Alberta",
                     PRV == 59 ~ "British Columbia"),
    PRV = factor(PRV)) 

summary(gss_main_2005$PRV) 
```

```{r REGION-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # REGION:  Region of residence of the respondent.
    REGION = 
      case_when(REGION == 1 ~ "Atlantic region",
                     REGION == 2 ~ "Quebec",
                     REGION == 3 ~ "Ontario",
                     REGION == 4 ~ "Prairie region",
                     REGION == 5 ~ "British Columbia"),
    REGION = factor(REGION)) 

summary(gss_main_2005$REGION)
```

```{r LUC_RST-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # LUC_RST:  Population centre indicator
    LUC_RST = 
      case_when(LUC_RST == 1 ~ "CMA/CA",
                     LUC_RST == 2 ~ "non CMA/CA",
                     LUC_RST == 3 ~ "Prince Edward Island"),
    LUC_RST = factor(LUC_RST)) 

summary(gss_main_2005$LUC_RST)
```


```{r GTU_Q110-2005}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never", 
                    "Don't know")

gss_main_2005 <- gss_main_2005 %>% 
  mutate( # GTU_Q110: How often do you feel rushed? Would you say it is...?
    GTU_Q110 = 
      case_when(GTU_Q110 == 1 ~ "Every day",
                     GTU_Q110 == 2 ~ "A few times a week",
                     GTU_Q110 == 3 ~ "About once a week",
                     GTU_Q110 == 4 ~ "About once a month",
                     GTU_Q110 == 5 ~ "Less than once a month",
                     GTU_Q110 == 6 ~ "Never",
                     GTU_Q110 == 9 ~ "Don't know"),
    GTU_Q110 = factor(GTU_Q110, levels = rev(values_ordered), ordered = TRUE)) 

summary(gss_main_2005$GTU_Q110)
```

```{r GTU_Q120-2005}
values_ordered <- c("More rushed",
                    "About the same",
                    "Less rushed",
                    "Don't know")

gss_main_2005 <- gss_main_2005 %>% 
  mutate( # GTU_Q120: How often do you feel you have time on your hands that you don’t know what to do with?
    GTU_Q120 = 
      case_when(GTU_Q120 == 1 ~ "More rushed",
                     GTU_Q120 == 2 ~  "About the same",
                     GTU_Q120 == 3 ~ "Less rushed",
                     GTU_Q120 == 9 ~ "Don't know"),
    GTU_Q120 = factor(GTU_Q120, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_2005$GTU_Q120)
```


```{r GTU_130-2005}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never", 
                    "Don't know")

gss_main_2005 <- gss_main_2005 %>% 
  mutate( # GTU_Q130: How often do you feel you have time on your hands that you don’t know what to do with?
    GTU_Q130 = 
      case_when(GTU_Q130 == 1 ~ "Every day",
                     GTU_Q130 == 2 ~ "A few times a week",
                     GTU_Q130 == 3 ~ "About once a week",
                     GTU_Q130 == 4 ~ "About once a month",
                     GTU_Q130 == 5 ~ "Less than once a month",
                     GTU_Q130 == 6 ~ "Never",
                     GTU_Q130 == 9 ~ "Don't know"),
    GTU_Q130 = factor(GTU_Q130, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_2005$GTU_Q130)
```

```{r GTU_Q140-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # GTU_Q140:  Do you feel that the days are just too short to do all the things you want?
    GTU_Q140 = 
           case_when(GTU_Q140 == 1 ~ "Yes", 
                     GTU_Q140 == 2 ~ "No",
                     GTU_Q140 == 8 ~ "Not stated", 
                     GTU_Q140 == 9 ~ "Don't know"), 
    GTU_Q140 = factor(GTU_Q140)) 

summary(gss_main_2005$GTU_Q140)
```

```{r GTU_Q150-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # GTU_Q150: Diary - Reference day - Type
    GTU_Q150 = 
      case_when(GTU_Q150 == 1 ~ "Time with family and friends (spouse, children, boy-friend - girl-friend)",
                     GTU_Q150 == 2 ~ "Relaxation - personal time",
                     GTU_Q150 == 3 ~ "Practising sports",
                     GTU_Q150 == 4 ~ "Crafts or hobbies",
                     GTU_Q150 == 5 ~ "Outdoor activities",
                     GTU_Q150 == 6 ~ "Reading - writing",
                     GTU_Q150 == 7 ~ "Studies",
                     GTU_Q150 == 8 ~ "Work",
                     GTU_Q150 == 9 ~ "Other",
                     GTU_Q150 == 10 ~ "Travel",
                     GTU_Q150 == 11 ~ "Volunteer",
                     GTU_Q150 == 12  ~ "Nothing",
                     GTU_Q150 == 13  ~ "Do household work",
                     GTU_Q150 == 98 ~ "Not stated",
                     GTU_Q150 == 99 ~ "Don't know",
),
    GTU_Q150 = factor(GTU_Q150)) 

summary(gss_main_2005$GTU_Q150)
```

```{r DVTDAY-2005}
gss_main_2005 <- gss_main_2005 %>% 
  mutate( # DVTDAY: Diary - Reference day - Type
    DVTDAY = 
      case_when(DVTDAY == 1 ~ "Weekday",
                     DVTDAY == 2 ~ "Saturday",
                     DVTDAY == 3 ~ "Sunday"),
    DVTDAY = factor(DVTDAY)) 

summary(gss_main_2005$DVTDAY)
```

Save data in R format for later use:
```{r save-data-2005}
usethis::use_data(gss_main_2005, 
                  overwrite = TRUE)
```

## GSS 1998

Read the Main files of the 1998 GSS survey:

```{r reading-1998-main-file}
# Main File
gss_m_1998 <- read.csv(paste0(here(), "/data-raw/source-files/Time-use-1998/gss-12M0012-E-1998-c-12-m_F1.csv"))
```

Select only the socioeconomic variables: 

```{r filtering-variables-1998}
gss_main_1998 <- gss_m_1998 %>%
  select(RECID:DVTDAY)
```

Labeling socioeconomic variables: 

```{r SURVMNTH-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # SURVMNTH: Survey month of data collection from February 1998 to January 1999
    SURVMNTH = 
           case_when(SURVMNTH == 1 ~ "January", 
                     SURVMNTH == 2 ~ "February",
                     SURVMNTH == 3 ~ "March",
                     SURVMNTH == 4 ~ "April",
                     SURVMNTH == 5 ~ "May",
                     SURVMNTH == 6  ~ "June",
                     SURVMNTH == 7 ~ "July",
                     SURVMNTH == 8 ~ "August",
                     SURVMNTH == 9 ~ "September",
                     SURVMNTH == 10 ~ "October",
                     SURVMNTH == 11 ~ "November",
                     SURVMNTH == 12 ~ "December"), 
    SURVMNTH = factor(SURVMNTH)) 

summary(gss_main_1998$SURVMNTH)
```

```{r LANINT-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # LANINT: Language of interview.
    LANINT = 
           case_when(LANINT == 1 ~ "English", 
                     LANINT == 2 ~ "French"), 
    LANINT = factor(LANINT)) 

summary(gss_main_1998$LANINT)
```

```{r AGEGR5-1998}
values_ordered <- c("15 to 17 years","18 to 19 years","20 to 24 years",
                    "25 to 29 years","30 to 34 years","35 to 39 year","40 to 44 years",
                    "45 to 49 years","50 to 54 years","55 to 59 year","60 to 64 years",
                    "65 to 69 years","70 to 74 years","75 to 79 year","80 years and over")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # AGEGR5: Age group of respondent (groups of 5)
    AGEGR5 = 
           case_when(AGEGR5 == 1 ~ "15 to 17 years",
                     AGEGR5 == 2 ~ "18 to 19 years",
                     AGEGR5 == 3 ~ "20 to 24 years",
                     AGEGR5 == 4 ~ "25 to 29 years",
                     AGEGR5 == 5 ~ "30 to 34 years",
                     AGEGR5 == 6  ~ "35 to 39 year",
                     AGEGR5 == 7 ~ "40 to 44 years",
                     AGEGR5 == 8 ~ "45 to 49 years",
                     AGEGR5 == 9 ~ "50 to 54 years",
                     AGEGR5 == 10 ~ "55 to 59 year",
                     AGEGR5 == 11 ~ "60 to 64 years",
                     AGEGR5 == 12 ~ "65 to 69 years",
                     AGEGR5 == 13 ~ "70 to 74 years",
                     AGEGR5 == 14 ~ "75 to 79 year",
                     AGEGR5 == 15 ~ "80 years and over"), 
    AGEGR5 = factor(AGEGR5, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$AGEGR5)
```

```{r AGEGR10-1998}
values_ordered <- c("15 to 24 years","25 to 34 years","35 to 44 years",
                    "45 to 54 years", "55 to 64 years","65 to 74 years","75 to 84 years", "85 years and over")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # AGEGR10: Age group of respondent (groups of 10)
    AGEGR10 = 
           case_when(AGEGR10 == 1 ~ "15 to 24 years", 
                     AGEGR10 == 2 ~ "25 to 34 years",
                     AGEGR10 == 3 ~ "35 to 44 years",
                     AGEGR10 == 4 ~ "45 to 54 years",
                     AGEGR10 == 5 ~ "55 to 64 years",
                     AGEGR10 == 6  ~ "65 to 74 years",
                     AGEGR10 == 7 ~ "75 to 84 years",
                     AGEGR10 == 8 ~ "85 years and over"), 
    AGEGR10 = factor(AGEGR10, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$AGEGR10)
```

```{r SEX-1998}
values_ordered <- c("Male", "Female")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # SEX: Sex of respondent
    SEX = 
           case_when(SEX == 1 ~ "Male", 
                     SEX == 2 ~ "Female"), 
    SEX = factor(SEX, levels = values_ordered, ordered = FALSE)) 

summary(gss_main_1998$SEX)
```

```{r MARSTAT-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # MARSTAT: Marital status of the respondent
    MARSTAT = 
           case_when(MARSTAT == 1 ~ "Married", 
                     MARSTAT == 2 ~ "Living common-law",
                     MARSTAT == 3 ~ "Widowed",
                     MARSTAT == 4 ~ "Separated",
                     MARSTAT == 5 ~ "Divorced",
                     MARSTAT == 6  ~ "Single, never married",
                     MARSTAT == 8 ~ "Don't know",
                     MARSTAT == 9 ~ "Not stated"), 
    MARSTAT = factor(MARSTAT))

summary(gss_main_1998$MARSTAT)
```

```{r AGEPRGR5-1998}
values_ordered <- c("Not Asked","15 to 19 years","20 to 24 years",
                    "25 to 29 years","30 to 34 years","35 to 39 year","40 to 44 years",
                    "45 to 49 years","50 to 54 years","55 to 59 year","60 to 64 years",
                    "65 to 69 years","70 to 74 years","75 to 79 year","80 years and over")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # AGEPRGR5: Age group of respondent (groups of 5)
    AGEPRGR5 = 
           case_when(AGEPRGR5 == 97 ~ "Not Asked",
                     AGEPRGR5 == 1 ~  "15 to 19 years",
                     AGEPRGR5 == 2 ~ "20 to 24 years",
                     AGEPRGR5 == 3 ~ "25 to 29 years",
                     AGEPRGR5 == 4 ~ "30 to 34 years",
                     AGEPRGR5 == 5  ~ "35 to 39 year",
                     AGEPRGR5 == 6 ~ "40 to 44 years",
                     AGEPRGR5 == 7 ~ "45 to 49 years",
                     AGEPRGR5 == 8 ~ "50 to 54 years",
                     AGEPRGR5 == 9 ~ "55 to 59 year",
                     AGEPRGR5 == 10 ~ "60 to 64 years",
                     AGEPRGR5 == 11 ~ "65 to 69 years",
                     AGEPRGR5 == 12 ~ "70 to 74 years",
                     AGEPRGR5 == 13 ~ "75 to 79 year",
                     AGEPRGR5 == 14 ~ "80 years and over"), 
    AGEPRGR5 = factor(AGEPRGR5, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$AGEPRGR5)
```

```{r PRTYPEC-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # PRTYPEC: Type of partner respondent has living in the household.
    PRTYPEC = 
           case_when(PRTYPEC == 1 ~ "Respondent has a married partner in the household", 
                     PRTYPEC == 2 ~ "Respondent has a common-law partner in the household",
                     PRTYPEC == 0 ~ "Respondent has no spouse/partner in the household"), 
    PRTYPEC = factor(PRTYPEC)) 

summary(gss_main_1998$PRTYPEC)
```

```{r CHRFLAG-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CHRFLAG:  Child(ren) of the respondent living in the household.
    CHRFLAG = 
           case_when(CHRFLAG == 1 ~ "Yes", 
                     CHRFLAG == 2 ~ "No"), 
    CHRFLAG = factor(CHRFLAG)) 

summary(gss_main_1998$CHRFLAG)
```

```{r AGECHRYC-1998}

values_ordered <- c(as.character(0:24),"25 and over","Not asked - no children in the household")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # AGECHRYC: Age of respondent's youngest child in household
           AGECHRYC = case_when(AGECHRYC == 25 ~ "25 and over",
                     AGECHRYC == 97 ~ "Not asked - no children in the household",
                     TRUE ~ as.character(AGECHRYC)), 
    AGECHRYC = factor(AGECHRYC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$AGECHRYC)
```

```{r CHR0004C-1998}
values_ordered <- c("None", "One child",
                     "Two or more children")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CHR0004C: Number of respondent’s children 0 to 4 years of age living in the household.
    CHR0004C = 
           case_when(CHR0004C == 0 ~ "None",
                     CHR0004C == 1 ~ "One child",
                     CHR0004C == 2 ~ "Two or more children"), 
    CHR0004C = factor(CHR0004C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$CHR0004C)
```


```{r CHR0512C-1998}
values_ordered <- c("None", "One",
                     "Two",
                     "Three or more children")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CHR0512C: Number of children aged from 5 to 12 years living in the respondent's household.
    CHR0512C = 
           case_when(CHR0512C == 0 ~ "None",
                     CHR0512C == 1 ~ "One",
                     CHR0512C == 2 ~ "Two",
                     CHR0512C == 3 ~ "Three or more children"), 
    CHR0512C = factor(CHR0512C, levels = values_ordered, ordered = TRUE)) 
summary(gss_main_1998$CHR0512C)
```


```{r CHR1314C-1998}
values_ordered <- c("None", "One child",
                     "Two or more children")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CHR1314C: Number of respondent’s children 13 to 14 years of age living in the household.
    CHR1314C = 
           case_when(CHR1314C == 0 ~ "None",
                     CHR1314C == 1 ~ "One child",
                     CHR1314C == 2 ~ "Two or more children"), 
    CHR1314C = factor(CHR1314C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$CHR1314C)
```

```{r CHR1518C-1998}
values_ordered <- c("None", "One child",
                     "Two or more children")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CHR1518C: Number of respondent’s children 15 to 18 years of age living in the household.
    CHR1518C = 
           case_when(CHR1518C == 0 ~ "None",
                     CHR1518C == 1 ~ "One child",
                     CHR1518C == 2 ~ "Two or more children"), 
    CHR1518C = factor(CHR1518C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$CHR1518C)
```

```{r CHR1924C-1998}
values_ordered <- c("None", "One child",
                     "Two or more children")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CHR1924C: Number of respondent’s children 19 to 24 years of age living in the household.
    CHR1924C = 
           case_when(CHR1924C == 0 ~ "None",
                     CHR1924C == 1 ~ "One child",
                     CHR1924C == 2 ~ "Two or more children"), 
    CHR1924C = factor(CHR1924C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$CHR1924C)
```

```{r CHR25UPC-1998}
values_ordered <- c("None", "One child",
                     "Two or more children")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CHR25UPC: Number of respondent’s children 25 years of age and older living in the household.
    CHR25UPC = 
           case_when(CHR25UPC == 0 ~ "None",
                     CHR25UPC == 1 ~ "One child",
                     CHR25UPC == 2 ~ "Two or more children"), 
    CHR25UPC = factor(CHR25UPC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$CHR25UPC)
```
```{r CHH0004C-1998}
values_ordered <- c("None", "One child",
                     "Two or more children")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CHH0004C: Number of children aged from 0 to 4 living in the respondent’s household.
    CHH0004C = 
           case_when(CHH0004C == 0 ~ "None",
                     CHH0004C == 1 ~ "One child",
                     CHH0004C == 2 ~ "Two or more children"), 
    CHH0004C = factor(CHH0004C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$CHH0004C)
```

```{r CHH1314C-1998}
values_ordered <- c("None", "One child",
                     "Two or more children")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CHH1314C: Number of children aged from 13 to 14 living in the respondent’s household.
    CHH1314C = 
           case_when(CHH1314C == 0 ~ "None",
                     CHH1314C == 1 ~ "One child",
                     CHH1314C == 2 ~ "Two or more children"), 
    CHH1314C = factor(CHH1314C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$CHH1314C)
```

```{r CHH0512C-1998}
values_ordered <- c("None", "One child",
                     "Two children","Three or more children")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CHH0512C: Number of children aged from 13 to 14 living in the respondent’s household.
    CHH0512C = 
           case_when(CHH0512C == 0 ~ "None",
                     CHH0512C == 1 ~ "One child",
                     CHH0512C == 2 ~ "Two children",
                     CHH0512C == 3 ~ "Three or more children",), 
    CHH0512C = factor(CHH0512C, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$CHH0512C)
```


```{r LIVARR08-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # LIVARR08: Living arrangement of respondent’s household (8 categories)
    LIVARR08 = 
           case_when(LIVARR08 == 1 ~ "Under 25 years - Not living with parents",
                     LIVARR08 == 2 ~ "Under 25 years - Living with both parents",
                     LIVARR08 == 3 ~ "Under 25 years - Living with mother only",
                     LIVARR08 == 4 ~ "Under 25 years - Living with father only",
                     LIVARR08 == 5 ~ "25 years of age or older - Not living with parents",
                     LIVARR08 == 6 ~ "25 years of age or older - Living with both parents",
                     LIVARR08 == 7 ~ "25 years of age or older - Living with mother only",
                     LIVARR08 == 8 ~ "25 years of age or older - Living with father only"), 
    LIVARR08 = factor(LIVARR08)) 

summary(gss_main_1998$LIVARR08)
```

```{r LIVARR12-1998} 
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # LIVARR12: Living arrangement of respondent’s household (12 categories)
    LIVARR12 = 
           case_when(LIVARR12 == 1 ~ "Living alone",
                     LIVARR12 == 2 ~ "Living with sponse only",
                     LIVARR12 == 3 ~ "Living with spouse and single child < 25 years",
                     LIVARR12 == 4 ~ "Living with spouse and single child 25 years or older",
                     LIVARR12 == 5 ~ "Living with spouse and non-child(ren)",
                     LIVARR12 == 6 ~ "Living with spouse and other",
                     LIVARR12 == 7 ~ "Living without spouse and single child  < 25 years",
                     LIVARR12 == 8 ~ "Living without spouse and single child 25 years or older",
                     LIVARR12 == 9 ~ "No spouse and non-single child(ren)",
                     LIVARR12 == 10 ~ "Living with two parents",
                     LIVARR12 == 11 ~ "Living with one parent",
                     LIVARR12 == 12 ~ "Other living arrangement"), 
    LIVARR12 = factor(LIVARR12)) 

summary(gss_main_1998$LIVARR12)
```

```{r HSDSIZEC-1998}
values_ordered <- c("One household member",
                   "Two household members",
                   "Three household members",
                   "Four household members",
                   "Five household members",
                   "Six household members",
                   "Seven household members", 
                   "Eight household members or more")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # HSDSIZEC: Household size of respondent
    HSDSIZEC = 
           case_when(HSDSIZEC == 1 ~ "One household member",
                     HSDSIZEC == 2 ~ "Two household members",
                     HSDSIZEC == 3 ~ "Three household members",
                     HSDSIZEC == 4 ~ "Four household members",
                     HSDSIZEC == 5 ~ "Five household members",
                     HSDSIZEC == 6 ~ "Six household members",
                     HSDSIZEC == 7 ~ "Seven household members",
                     HSDSIZEC == 8 ~ "Eight household members or more"), 
    HSDSIZEC = factor(HSDSIZEC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1998$HSDSIZEC)
```

```{r FAMTYPE-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # FAMTYPE: Respondent's type of family structure.
    FAMTYPE = 
           case_when(FAMTYPE == 1 ~ "Couple only", 
                     FAMTYPE == 2 ~ "Intact family",
                     FAMTYPE == 3 ~ "Step family with common child",
                     FAMTYPE == 4 ~ "Step family without a common child",
                     FAMTYPE == 5 ~ "Lone parent family",
                     FAMTYPE == 7 ~ "Not asked"), 
    FAMTYPE = factor(FAMTYPE))

summary(gss_main_1998$FAMTYPE)
```

```{r MULTIGEN-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # MULTIGEN:  Three generations or more in the respondent’s household
    MULTIGEN = 
           case_when(MULTIGEN == 1 ~ "Yes", 
                     MULTIGEN == 2 ~ "No"), 
    MULTIGEN = factor(MULTIGEN))

summary(gss_main_1998$MULTIGEN)
```


```{r PRV-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # PRV: Province of residence
    PRV = 
      case_when(PRV == 10 ~ "Newfoundland and Labrador",
                     PRV == 11 ~ "Prince Edward Island",
                     PRV == 12 ~ "Nova Scotia",
                     PRV == 13 ~ "New Brunswick",
                     PRV == 24 ~ "Quebec",
                     PRV == 35 ~ "Ontario",
                     PRV == 46 ~ "Manitoba",
                     PRV == 47 ~ "Saskatchewan",
                     PRV == 48 ~ "Alberta",
                     PRV == 59 ~ "British Columbia"),
    PRV = factor(PRV)) 

summary(gss_main_1998$PRV) 
```

```{r REGION-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # REGION:  Region of residence of the respondent.
    REGION = 
      case_when(REGION == 1 ~ "Atlantic region",
                     REGION == 2 ~ "Quebec",
                     REGION == 3 ~ "Ontario",
                     REGION == 4 ~ "Prairie region",
                     REGION == 5 ~ "British Columbia"),
    REGION = factor(REGION)) 

summary(gss_main_1998$REGION)
```

```{r CMAPRV-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # CMAPRV:  Population centre indicator
    CMAPRV = 
      case_when(CMAPRV %in% c(6,8,11,13,15) ~ "CMA",
                     CMAPRV %in% c(1,2,3,4,5,7,9,10,12,14) ~ "non CMA"),
    CMAPRV = factor(CMAPRV)) 

summary(gss_main_1998$CMAPRV)
```

```{r A2-1998}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never", 
                    "Not stated")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # A2: How often do you feel rushed?
    A2 = 
      case_when(A2 == 1 ~ "Every day",
                     A2 == 2 ~ "A few times a week",
                     A2 == 3 ~ "About once a week",
                     A2 == 4 ~ "About once a month",
                     A2 == 5 ~ "Less than once a month",
                     A2 == 6 ~ "Never",
                     A2 == 9 ~ "Not stated"),
    A2 = factor(A2, levels = rev(values_ordered), ordered = TRUE)) 

summary(gss_main_1998$A2)
```

```{r A3-1998}
values_ordered <- c("More rushed",
                    "About the same",
                    "Less rushed",
                    "Don't know","Not stated")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # A3: Compared to five years ago, do you feel...
    A3 = 
      case_when(A3 == 1 ~ "More rushed",
                     A3 == 2 ~  "About the same",
                     A3 == 3 ~ "Less rushed",
                     A3 == 8 ~ "Don't know",
                     A3 == 9 ~ "Not stated"),
    A3 = factor(A3, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_1998$A3)
```

```{r GTU_130-1998}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never", 
                    "Not stated")

gss_main_1998 <- gss_main_1998 %>% 
  mutate( # A4: How often do you feel you have time on your hands that you don’t know what to do with?
    A4 = 
      case_when(A4 == 1 ~ "Every day",
                     A4 == 2 ~ "A few times a week",
                     A4 == 3 ~ "About once a week",
                     A4 == 4 ~ "About once a month",
                     A4 == 5 ~ "Less than once a month",
                     A4 == 6 ~ "Never",
                     A4 == 9 ~ "Not stated"),
    A4 = factor(A4, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_1998$A4)
```

```{r A5-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # A5:  Do you feel that the days are just too short to do all the things you want?
    A5 = 
           case_when(A5 == 1 ~ "Yes", 
                     A5 == 2 ~ "No",
                     A5 == 9 ~ "Not stated", 
                     A5 == 8  ~ "Don't know"), 
    A5 = factor(A5)) 

summary(gss_main_1998$A5)
```

```{r A6-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # A6: On which main activity would you choose to spend more time on
    A6 = 
      case_when(A6 == 1 ~ "Time with family and friends (spouse, children, boy-friend - girl-friend)",
                     A6 == 2 ~ "Relaxation - personal time",
                     A6 == 3 ~ "Practising sports",
                     A6 == 4 ~ "Crafts or hobbies",
                     A6 == 5 ~ "Outdoor activities",
                     A6 == 6 ~ "Reading - writing",
                     A6 == 7 ~ "Studies",
                     A6 == 8 ~ "Work",
                     A6 == 9 ~ "Domestic work",
                     A6 == 10 ~ "Holiday/Travel",
                     A6 == 11 ~ "Volunteer",
                     A6 == 12  ~ "Other",
                     A6 == 97  ~ "Not asked",
                     A6 == 98 ~ "Not stated",
                     A6 == 99 ~ "Don't know",
),
    A6 = factor(A6)) 

summary(gss_main_1998$A6)
```

```{r DDAY-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # DDAY: Diary - Reference day - Type
    DDAY = 
      case_when(DDAY == 1 ~ "Sunday",
                     DDAY == 2 ~ "Monday",
                     DDAY == 3 ~ "Tuesday",
                     DDAY == 4 ~ "Wednesday",
                     DDAY == 5 ~ "Thursday",
                     DDAY == 6 ~ "Friday",
                     DDAY == 7 ~ "Saturday"),
    DDAY = factor(DDAY)) 

summary(gss_main_1998$DDAY)
```

```{r DIARYREC-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # DIARYREC: Recall of diary interview
    DIARYREC = 
      case_when(DIARYREC == 1 ~ "Diary completed the day following the designated day (24 hrs recall)",
                     DIARYREC == 2 ~ "Diary completed the 2nd day following the designated day (48 hrs recall)",
                     DIARYREC == 3 ~ "Diary completed in 2 days (24 to 48 hrs recall)"),
    DIARYREC = factor(DIARYREC)) 

summary(gss_main_1998$DIARYREC)
```

```{r DVTDAY-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # DVTDAY: Diary - Reference day - Type
    DVTDAY = 
      case_when(DVTDAY == 1 ~ "Weekday",
                     DVTDAY == 2 ~ "Saturday",
                     DVTDAY == 3 ~ "Sunday"),
    DVTDAY = factor(DVTDAY)) 

summary(gss_main_1998$DVTDAY)
```

```{r PARHSDC-1998}
gss_main_1998 <- gss_main_1998 %>% 
  mutate( # PARHSD: Type of parents the respondent has within the household.
    PARHSD = 
           case_when(PARHSD == 1 ~ "Two biological parents",
                     PARHSD == 2 ~ "Biologic parents",
                     PARHSD == 3 ~ "Biologic-parents",
                     PARHSD == 4 ~ "Only one parent",
                     PARHSD == 7 ~ "Not asked - no parents in the household"), 
    PARHSD = factor(PARHSD)) 

summary(gss_main_1998$PARHSD)
```

Save data in R format for later use:
```{r save-data-1998}
usethis::use_data(gss_main_1998, 
                  overwrite = TRUE)
```

## GSS 1992

Read the Main files of the 1992 GSS survey:

```{r reading-1992-main-file}
# Main File
gss_m_1992 <- read.csv(paste0(here(), "/data-raw/source-files/Time-use-1992/gss-12M0007-E-1992-c-7-m_F1.csv"))
```

Select only the socioeconomic variables: 

```{r filtering-variables-1998}
gss_main_1992 <- gss_m_1992 %>%
  select(c("SEQNUM", 
 "FWGHT", 
 "SURMNTH", 
 "FORMTYPE",
 "A4", 
 "A5", 
 "A6",
 "C2", 
 "DVAGEGR", 
 "DVSEX", 
 "DVMS", 
 "DVHHSCAP",
 "DVPROV", 
 "DVCMA", 
 "DVLVGRGR", 
 "DVAGRYC", 
 "RESC0004", 
 "RESC0512", 
 "RESC1318", 
 "RESC1924", 
 "RESC2596", 
 "DVAGRSP"))
```

Labeling socioeconomic variables: 

```{r C2-1992}
gss_main_1992 <- gss_main_1992 %>% 
  mutate( # C2:  Respondent has child(ren) <15?
    C2 = 
           case_when(C2 == 1 ~ "Yes", 
                     C2 == 2 ~ "No"), 
    C2 = factor(C2)) 

summary(gss_main_1992$C2)
```

```{r SURMNTH-1992}
gss_main_1992 <- gss_main_1992 %>% 
  mutate( # SURMNTH: Survey month of data collection from April 1992 to March 2016
    SURMNTH = 
           case_when(SURMNTH == 1 ~ "January", 
                     SURMNTH == 2 ~ "February",
                     SURMNTH == 3 ~ "March",
                     SURMNTH == 4 ~ "April",
                     SURMNTH == 5 ~ "May",
                     SURMNTH == 6  ~ "June",
                     SURMNTH == 7 ~ "July",
                     SURMNTH == 8 ~ "August",
                     SURMNTH == 9 ~ "September",
                     SURMNTH == 10 ~ "October",
                     SURMNTH == 11 ~ "November",
                     SURMNTH == 12 ~ "December"), 
    SURMNTH = factor(SURMNTH)) 

summary(gss_main_1992$SURMNTH)
```

```{r FORMTYPE-1992}
gss_main_1992 <- gss_main_1992 %>% 
  mutate( # FORMTYPE: Language of interview.
    FORMTYPE = 
           case_when(FORMTYPE == 1 ~ "English", 
                     FORMTYPE == 2 ~ "French"), 
    FORMTYPE = factor(FORMTYPE)) 

summary(gss_main_1992$FORMTYPE)
```

```{r A4-1992}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never", 
                    "Not stated")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # A4: How often do you feel rushed?
    A4 = 
      case_when(A4 == 1 ~ "Every day",
                     A4 == 2 ~ "A few times a week",
                     A4 == 3 ~ "About once a week",
                     A4 == 4 ~ "About once a month",
                     A4 == 5 ~ "Less than once a month",
                     A4 == 6 ~ "Never",
                     A4 == 9 ~ "Not stated"),
    A4 = factor(A4, levels = rev(values_ordered), ordered = TRUE)) 

summary(gss_main_1992$A4)
```

```{r A5-1992}
values_ordered <- c("More rushed",
                    "About the same",
                    "Less rushed","Not stated")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # A5: Compared to five years ago, do you feel...
    A5 = 
      case_when(A5 == 1 ~ "More rushed",
                     A5 == 2 ~  "About the same",
                     A5 == 3 ~ "Less rushed",
                     A5 == 9 ~ "Not stated"),
    A5 = factor(A5, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_1992$A5)
```

```{r GTU_130-1992}
values_ordered <- c("Every day",
                    "A few times a week",
                    "About once a week",
                    "About once a month",
                    "Less than once a month",
                    "Never", 
                    "Not stated")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # A6: How often do you feel you have time on your hands that you don’t know what to do with?
    A6 = 
      case_when(A6 == 1 ~ "Every day",
                     A6 == 2 ~ "A few times a week",
                     A6 == 3 ~ "About once a week",
                     A6 == 4 ~ "About once a month",
                     A6 == 5 ~ "Less than once a month",
                     A6 == 6 ~ "Never",
                     A6 == 9 ~ "Not stated"),
    A6 = factor(A6, levels = rev(values_ordered), ordered = TRUE))  

summary(gss_main_1992$A6)
```

```{r DVAGEGR-1992}
values_ordered <- c("15 to 17 years","18 to 19 years","20 to 24 years",
                    "25 to 29 years","30 to 34 years","35 to 39 year","40 to 44 years",
                    "45 to 49 years","50 to 54 years","55 to 59 year","60 to 64 years",
                    "65 to 69 years","70 to 74 years","75 to 79 year","80 years and over")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # DVAGEGR: Age group of respondent (groups of 5)
    DVAGEGR = 
           case_when(DVAGEGR == 1 ~ "15 to 17 years",
                     DVAGEGR == 2 ~ "18 to 19 years",
                     DVAGEGR == 3 ~ "20 to 24 years",
                     DVAGEGR == 4 ~ "25 to 29 years",
                     DVAGEGR == 5 ~ "30 to 34 years",
                     DVAGEGR == 6  ~ "35 to 39 year",
                     DVAGEGR == 7 ~ "40 to 44 years",
                     DVAGEGR == 8 ~ "45 to 49 years",
                     DVAGEGR == 9 ~ "50 to 54 years",
                     DVAGEGR == 10 ~ "55 to 59 year",
                     DVAGEGR == 11 ~ "60 to 64 years",
                     DVAGEGR == 12 ~ "65 to 69 years",
                     DVAGEGR == 13 ~ "70 to 74 years",
                     DVAGEGR == 14 ~ "75 to 79 year",
                     DVAGEGR == 15 ~ "80 years and over"), 
    DVAGEGR = factor(DVAGEGR, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1992$DVAGEGR)
```

```{r DVSEX-1992}
values_ordered <- c("Male", "Female")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # DVSEX: DVSEX of respondent
    DVSEX = 
           case_when(DVSEX == 1 ~ "Male", 
                     DVSEX == 2 ~ "Female"), 
    DVSEX = factor(DVSEX, levels = values_ordered, ordered = FALSE)) 

summary(gss_main_1992$DVSEX)
```

```{r DVMS-1992}
gss_main_1992 <- gss_main_1992 %>% 
  mutate( # DVMS: Marital status of the respondent
    DVMS = 
           case_when(DVMS == 1 ~ "Married or common-law",
                     DVMS == 2 ~ "Widowed",
                     DVMS == 3 ~ "Separated or Divorced",
                     DVMS == 4  ~ "Single, never married",
                     DVMS == 9 ~ "Not stated"), 
    DVMS = factor(DVMS))

summary(gss_main_1992$DVMS)
```

```{r DVHHSCAP-1992}
values_ordered <- c("One household member",
                   "Two household members",
                   "Three household members",
                   "Four household members",
                   "Five household members",
                   "Six household members",
                   "Seven members or more")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # DVHHSCAP: Household size of respondent
    DVHHSCAP = 
           case_when(DVHHSCAP == 1 ~ "One household member",
                     DVHHSCAP == 2 ~ "Two household members",
                     DVHHSCAP == 3 ~ "Three household members",
                     DVHHSCAP == 4 ~ "Four household members",
                     DVHHSCAP == 5 ~ "Five household members",
                     DVHHSCAP == 6 ~ "Six household members",
                     DVHHSCAP == 7 ~ "Seven members or more"), 
    DVHHSCAP = factor(DVHHSCAP, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1992$DVHHSCAP)
```

```{r DVPROV-1992}
gss_main_1992 <- gss_main_1992 %>% 
  mutate( # DVPROV: Province of residence
    DVPROV = 
      case_when(DVPROV == 0 ~ "Newfoundland and Labrador",
                     DVPROV == 1 ~ "Prince Edward Island",
                     DVPROV == 2 ~ "Nova Scotia",
                     DVPROV == 3 ~ "New Brunswick",
                     DVPROV == 4 ~ "Quebec",
                     DVPROV == 5 ~ "Ontario",
                     DVPROV == 6 ~ "Manitoba",
                     DVPROV == 7 ~ "Saskatchewan",
                     DVPROV == 8 ~ "Alberta",
                     DVPROV == 9 ~ "British Columbia"),
    DVPROV = factor(DVPROV)) 

summary(gss_main_1992$DVPROV) 
```

```{r DVCMA-1992}
gss_main_1992 <- gss_main_1992 %>% 
  mutate( # DVCMA:  Population centre indicator
    DVCMA = 
      case_when(DVCMA %in% c(6,8,11,13,15) ~ "CMA",
                     DVCMA %in% c(1,2,3,4,5,7,9,10,12,14) ~ "non CMA"),
    DVCMA = factor(DVCMA)) 

summary(gss_main_1992$DVCMA)
```
```{r DVLVGRGR-1992} 
gss_main_1992 <- gss_main_1992 %>% 
  mutate( # DVLVGRGR: Living arrangement of respondent’s household (12 categories)
    DVLVGRGR = 
           case_when(DVLVGRGR == 1 ~ "Living alone",
                     DVLVGRGR == 2 ~ "Living with sponse only",
                     DVLVGRGR == 3 ~ "Living with spouse and single child < 25 years",
                     DVLVGRGR == 4 ~ "Living with spouse and single child 25 years or older",
                     DVLVGRGR == 5 ~ "Living with spouse and non-child(ren)",
                     DVLVGRGR == 6 ~ "Living with spouse and other",
                     DVLVGRGR == 7 ~ "Living without spouse and single child  < 25 years",
                     DVLVGRGR == 8 ~ "Living without spouse and single child 25 years or older",
                     DVLVGRGR == 9 ~ "No spouse and non-single child(ren)",
                     DVLVGRGR == 10 ~ "Living with two parents",
                     DVLVGRGR == 11 ~ "Living with one parent",
                     DVLVGRGR == 12 ~ "Other living arrangement"), 
    DVLVGRGR = factor(DVLVGRGR)) 

summary(gss_main_1992$DVLVGRGR)
```

```{r RESC0004C-1992}
values_ordered <- c("None", "One child",
                     "Two or more children")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # RESC0004: Number of respondents single children aged 0-4.
    RESC0004 = 
           case_when(RESC0004 == 0 ~ "None",
                     RESC0004 == 1 ~ "One child",
                     RESC0004 == 2 ~ "Two or more children"), 
    RESC0004 = factor(RESC0004, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1992$RESC0004)
```
```{r RESC0512C-1992}
values_ordered <- c("None", "One",
                     "Two",
                     "Three or more children")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # RESC0512: Number of children aged from 5 to 12 years living in the respondent's household.
    RESC0512 = 
           case_when(RESC0512 == 0 ~ "None",
                     RESC0512 == 1 ~ "One",
                     RESC0512 == 2 ~ "Two",
                     RESC0512 == 3 ~ "Three or more children"), 
    RESC0512 = factor(RESC0512, levels = values_ordered, ordered = TRUE)) 
summary(gss_main_1992$RESC0512)
```


```{r RESC1318-1992}
values_ordered <- c("None", "One child",
                     "Two children", "Three or more children")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # RESC1318: Number of respondent’s children 15 to 18 years of age living in the household.
    RESC1318 = 
           case_when(RESC1318 == 0 ~ "None",
                     RESC1318 == 1 ~ "One child",
                     RESC1318 == 2 ~ "Two children", 
                     RESC1318 == 3 ~ "Three or more children"), 
    RESC1318 = factor(RESC1318, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1992$RESC1318)
```

```{r RESC1924-1992}
values_ordered <- c("None", "One child",
                     "Two or more children")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # RESC1924: Number of respondent’s children 19 to 24 years of age living in the household.
    RESC1924 = 
           case_when(RESC1924 == 0 ~ "None",
                     RESC1924 == 1 ~ "One child",
                     RESC1924 == 2 ~ "Two or more children"), 
    RESC1924 = factor(RESC1924, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1992$RESC1924)
```

```{r RESC2596C-1992}
values_ordered <- c("None", "One child",
                     "Two or more children")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # RESC2596: Number of respondent’s children 25 years of age and older living in the household.
    RESC2596 = 
           case_when(RESC2596 == 0 ~ "None",
                     RESC2596 == 1 ~ "One child",
                     RESC2596 == 2 ~ "Two or more children"), 
    RESC2596 = factor(RESC2596, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1992$RESC2596)
```

```{r DVAGRSP-1992}
values_ordered <- c("SPOUSE NOT LIVING IN HOUSEHOLD",
                    "NOT APPLICABLE",
                    "NOT STATED",
                    "15 to 24 years","25 to 34 years","35 to 44 years",
                    "45 to 54 years", "55 to 64 years","65 to 74 years","75 years and over")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # DVAGRSP: Age group of the respondents spouse.
    DVAGRSP = 
           case_when(DVAGRSP == 1 ~ "15 to 24 years", 
                     DVAGRSP == 2 ~ "25 to 34 years",
                     DVAGRSP == 3 ~ "35 to 44 years",
                     DVAGRSP == 4 ~ "45 to 54 years",
                     DVAGRSP == 5 ~ "55 to 64 years",
                     DVAGRSP == 6  ~ "65 to 74 years",
                     DVAGRSP == 7 ~ "75 years and over",
                     DVAGRSP == 8 ~ "SPOUSE NOT LIVING IN HOUSEHOLD", 
                     DVAGRSP == 0 ~ "NOT APPLICABLE", 
                     DVAGRSP == 9 ~ "NOT STATED"), 
    DVAGRSP = factor(DVAGRSP, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1992$DVAGRSP)
```
```{r DVAGRYC-1992}
values_ordered <- c(as.character(0:24),"25 and over","NOT APPLICABLE", "SINGLE CHILD OLDER THAN 25")

gss_main_1992 <- gss_main_1992 %>% 
  mutate( # DVAGRYC: Age of respondent's youngest child in household
           DVAGRYC = case_when(DVAGRYC == 25 ~ "25 and over",
                     DVAGRYC == 97 ~ "NOT APPLICABLE",
                     DVAGRYC == 98 ~ "SINGLE CHILD OLDER THAN 25",
                     TRUE ~ as.character(DVAGRYC)), 
    DVAGRYC = factor(DVAGRYC, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1992$DVAGRYC)
```



Save data in R format for later use:
```{r save-data-1992}
usethis::use_data(gss_main_1992, 
                  overwrite = TRUE)
```


## GSS 1986

Read the Main files of the 1986 GSS survey:

```{r reading-1986-main-file}
# Main File
gss_m_1986 <- read.csv(paste0(here(), "/data-raw/source-files/Time-use-1986/gss-12M0002-E-1986-c-2-main_F1.csv"))
```

Select only the socioeconomic variables: 

```{r filtering-variables-1986}
gss_main_1986 <- gss_m_1986 %>%
  select(c("SEQNUM", 
 "FWGT_OS", 
 "DVAGEGR", 
 "DVSEX", 
 "DVMARST",
 "DVHHLDSZ",
 "PROV", 
 "DVCHILD"))
```

Labeling socioeconomic variables: 

```{r C2-1986}
gss_main_1986 <- gss_main_1986 %>% 
  mutate( # DVCHILD: NUMBER OF CHILDREN AT HOME
    DVCHILD = 
           case_when(DVCHILD == 1 ~ "NONE < 19", 
                     DVCHILD == 2 ~ "ALL < 5",
                     DVCHILD == 3~ "4 < ALL < 13",
                     DVCHILD == 4 ~ "12 < ALL < 19",
                     DVCHILD == 5 ~ "SOME < 5",
                     DVCHILD == 6 ~ "OTHER"), 
    DVCHILD = factor(DVCHILD)) 

summary(gss_main_1986$DVCHILD)
```

```{r DVAGEGR-1986}
values_ordered <- c("15 to 19 years","20 to 24 years",
                    "25 to 29 years","30 to 34 years","35 to 39 year","40 to 44 years",
                    "45 to 49 years","50 to 54 years","55 to 59 year","60 to 64 years",
                    "65 to 69 years","70 to 74 years","75 to 79 year","80 years and over")

gss_main_1986 <- gss_main_1986 %>% 
  mutate( # DVAGEGR: Age group of respondent (groups of 5)
    DVAGEGR = 
           case_when(DVAGEGR == 1 ~ "15 to 19 years",
                     DVAGEGR == 2 ~ "20 to 24 years",
                     DVAGEGR == 3 ~ "25 to 29 years",
                     DVAGEGR == 4 ~ "30 to 34 years",
                     DVAGEGR == 5  ~ "35 to 39 year",
                     DVAGEGR == 6 ~ "40 to 44 years",
                     DVAGEGR == 7 ~ "45 to 49 years",
                     DVAGEGR == 8 ~ "50 to 54 years",
                     DVAGEGR == 9 ~ "55 to 59 year",
                     DVAGEGR == 10 ~ "60 to 64 years",
                     DVAGEGR == 11 ~ "65 to 69 years",
                     DVAGEGR == 12 ~ "70 to 74 years",
                     DVAGEGR == 13 ~ "75 to 79 year",
                     DVAGEGR == 1 ~ "80 years and over"), 
    DVAGEGR = factor(DVAGEGR, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1986$DVAGEGR)
```

```{r DVSEX-1986}
values_ordered <- c("Male", "Female")

gss_main_1986 <- gss_main_1986 %>% 
  mutate( # DVSEX: DVSEX of respondent
    DVSEX = 
           case_when(DVSEX == 1 ~ "Male", 
                     DVSEX == 2 ~ "Female"), 
    DVSEX = factor(DVSEX, levels = values_ordered, ordered = FALSE)) 

summary(gss_main_1986$DVSEX)
```

```{r DVMARST-1986}
gss_main_1986 <- gss_main_1986 %>% 
  mutate( # DVMARST: Marital status of the respondent
    DVMARST = 
           case_when(DVMARST == 1 ~ "Married",
                     DVMARST == 3 ~ "Widowed",
                     DVMARST == 4 ~ "Separated or Divorced",
                     DVMARST == 2  ~ "Single",
                     DVMARST == 9 ~ "Not stated"), 
    DVMARST = factor(DVMARST))

summary(gss_main_1986$DVMARST)
```

```{r DVHHLDSZ-1986}
values_ordered <- c("One household member",
                   "Two household members",
                   "Three household members",
                   "Four members or more")

gss_main_1986 <- gss_main_1986 %>% 
  mutate( # DVHHLDSZ: Household size of respondent
    DVHHLDSZ = 
           case_when(DVHHLDSZ == 1 ~ "One household member",
                     DVHHLDSZ == 2 ~ "Two household members",
                     DVHHLDSZ == 3 ~ "Three household members",
                     DVHHLDSZ == 4 ~ "Four members or more"), 
    DVHHLDSZ = factor(DVHHLDSZ, levels = values_ordered, ordered = TRUE)) 

summary(gss_main_1986$DVHHLDSZ)
```

```{r PROV-1986}
gss_main_1986 <- gss_main_1986 %>% 
  mutate( # PROV: Province of residence
    PROV = 
      case_when(PROV == 0 ~ "Newfoundland and Labrador",
                     PROV == 1 ~ "Prince Edward Island",
                     PROV == 2 ~ "Nova Scotia",
                     PROV == 3 ~ "New Brunswick",
                     PROV == 4 ~ "Quebec",
                     PROV == 5 ~ "Ontario",
                     PROV == 6 ~ "Manitoba",
                     PROV == 7 ~ "Saskatchewan",
                     PROV == 8 ~ "Alberta",
                     PROV == 9 ~ "British Columbia"),
    PROV = factor(PROV)) 

summary(gss_main_1986$PROV) 
```
Save data in R format for later use:
```{r}
usethis::use_data(gss_main_1986, 
                  overwrite = TRUE)
```
