...
For {ActiveCA}, data we extracted all data needed to characterize active travel 
...
<!-- 
is an `R` data package that provides analysis-ready data from the General Social Survey of Canada  [@statisticscanada2024] regarding active travel. These data can be used to study walking and cycling in Canada for various purposes over time, and to calculate impedance functions for accessibility analysis. The package provides pre-estimated impedance functions for cycling and walking trips for Census Agglomerations and Census Metropolitan Areas.
-->
...
<!-- 
Maybe here provide an example of the structure of this table?
-->
...
For each selected cycle of the GSS surveys, we reviewed the episode files to 
identify episodes of movement that involved walking or cycling. This allowed us 
to select the activities immediately before and after the movement episode. After
that, we labeled the code variables with their appropriate descriptions, identifying
each origin and destination, mode of travel, as well the province and urban 
classification of the episode. 

<!-- 

Show a graphical example of this process: how were active trips identified? 
What was the purpose of finding the preceding and succeeding activities?
...
The paper does not really need to report "Results". There are too many potential use cases and we would rather let users find their own results depending on what they are interested in. Instead of results, a section on "How to use the package" is appropriate. There are a number of points to highlight here, for example, how to load the package, call the data sets, in particular the use of episode weights to make sure that any statistical analysis is valid.
