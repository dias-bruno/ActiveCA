...
Similar to Table \ref{tab:main-2015-unprocessed}, which displayed an example of the main file structure, Table \ref{tab:ep-2015-unprocessed} presents the first seven episodes for the record identification number `10041` and some variables from the GSS PUMF 2015 episode file (Cycle 29). Each row in the table corresponds to an episode associated with the specified record identification (`PUMFID = 10041`), including the episode's weight (`WGHT_EPI`), the episode number (`EPINO`), the activity code (`TUI_01`), the episode's duration (`DURATION`), and the episode's location (`LOCATION`).

In total, the episode file of the 2015 GSS surveys contains 274,108 records, representing `r sum(gss_e_2015$WGHT_EPI)` episodes and 527 variables that detail the episodes. Similar to the main file, Statistics Canada has created codes for the discrete variables, where each value has a corresponding label.

In the case illustrated in Table \ref{tab:ep-2015-unprocessed}, this respondent began the diary description while sleeping at home (`TUI_01 = 1` and `LOCATION = 300`) for 210 minutes, followed by 40 minutes of personal hygiene (`TUI_01 = 2`). The respondent then spent 15 minutes on personal care activities, such as getting ready for school, supervising homework, reading, playing, reprimanding, or providing educational or emotional support, as indicated by `TUI_01 = 27`. Next, they recorded a travel episode, walking for 15 minutes (`TUI_01 = 7` and `LOCATION = 315`), where the origin and destination were both their home. Such trips, where the journey starts and finishes at home, are classified as recreational and leisure trips.

In sequence, the respondent spent 3 hours searching for a job (`TUI_01 = 9`), took a 15-minute lunch break (`TUI_01 = 6`), and then cleaned the house (`TUI_01 = 18`) for two hours. Table \ref{tab:ep-2015-unprocessed} displays only six variables out of the 527 available. As shown, since the dataset does not label the variable's values, decoding the values for each variable can be a time-consuming and a challenging task.

```{r gss-epi-file-2015}
main_2015_unprocessed <- gss_e_2015 |>
...
The active trips (walking and cycling) were identified by the their correspondent activity code, considering the differences between the survey cycles. Identifying the proceeding and succeeding activities of each activity episode helps to understand the motivation of the trip and also enables to obtain the origin and destination of the trips.

For each GSS main files, we selected some socioeconomic variables for helping profiling the individuals engaged in active trip episodes. We selected valuable socioeconomic variables, such as age group of the respondent, sex of the respondent, marital status, number of children, between many others. After that, and similar to as we did to episode files, we labeled the code variables with their appropriate descriptions. 

# How to use `ActiveCA`

The code chunk below demonstrates how you can install the `ActiveCA` in R:
...
After installing `ActiveCA` R package, you can load the package according to the following code:

```{r load-ActiveCA, warning=FALSE}
library(ActiveCA)
```

By running `?ActiveCA` in `R`, you can access all the package documentation. The documentation includes the description of the package, information about the authors and maintainers, and visualize all data sets already processed included in the package.

Figure X, display the documentation for the walking episodes of the 2015 GSS (`walking_2015.rda`). The documentation provides a brief description of the data set, how to load the package, also including the description of the format of the file and the description of all variables (already labeled). 


After restart your R session, you can access the data set by loading each of them. The chunk below displays how to load 2015 GSS episodes cycles. 
...
{ActiveCA} also enables visual analysis of active travel in Canada using traditional 
exploratory data analysis techniques. Figures \ref{fig:figure-01} and \ref{fig:figure-02}
show walking and cycling trips from 1992 and 2015 through heat maps. These maps 
use color gradients to represent the percentage of trips between various origins
and destinations, with darker colors indicating higher percentages and lighter 
colors representing less frequent routes. For conciseness, we omitted the heat maps
for the other years analyzed.
...
pre-estimated impedance functions for cycling and walking trips for Census Agglomerations and Census Metropolitan Areas. Table \ref{tab:table-04} shows the best impedance functions for walking trips to `Work or school` across survey years, where all distributions, except for a gamma function in 1998, are log-normal.
...
selected_functions <- gss_impedances_renamed %>%
  filter(dest_label == destino, MODE %in% mode) %>%
  ...
  unique() %>% 
  arrange(Mode, Year) %>%
  select(Mode, Year, Function, est_1, est_2, AIC, count)

selected_functions_tab <- selected_functions %>% 
  kable(format = "latex",
        booktabs = TRUE,
        caption = paste0("\\label{table-04}Impedance functions and AIC for `", mode ,"` trips considering `", destino,"` as destination."), 
        col.names = c("Mode", "Year", "Impedance function","Parameter 1*","Parameter 2*", "AIC","Count")) %>% 
    kable_styling(full_width = FALSE, 
                latex_options=c("scale_down"),
                font_size = 10) %>% 
  row_spec(0, bold = TRUE, align = "c") %>% 
...
accessibility measures for active travel across various destinations and temporal 
scales in Canadian urban areas.
...
combined_plot <- gss_impedances_renamed %>% 
  ...
  # Case of only one year 
    if(plots == 1){
      ggsave(file = paste0(here::here(),"/paper/Manuscript-figures/impf_",destino,".jpg"), 
       plot = combined_plot,
       width = 7, 
       height = 4, 
       units = "in", 
...
```{r figure-03, echo=FALSE, out.width="100%", fig.cap=paste0("Empirical data and impedance functions fitted for `work or school` as destination."), fig.align="center"}

knitr::include_graphics(paste0(here::here(),"/paper/Manuscript-figures/impf_Work or school.jpg"))
```
...
Additionally, the package includes a series of pre-estimated impedance functions 
for walking and cycling trips, considering various destinations and time periods, 
for Canadian Metropolitan and Census Agglomeration areas.
...
can seamlessly explore GSS walking and cycling episodes <!-- along with calibrated impedance functions-->,
