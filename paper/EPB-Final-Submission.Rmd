...
corrauth: "Bruno Dias dos Santos - School of Earth, Environment and Society, McMaster University, 1280 Main St W, Hamilton, Ontario L8S 4K1 Canada."
...
product with data from all Time Use Survey (TUS) cycles of the Canadian General
...
is done on the topic of time use. Concretely, {ActiveCA} covers
...
urban, and health policy. The rest of this paper discusses the sources of data, and the process
implemented to retrieve and package them. Then, we show some selected examples of analysis to whet the
...
conditions and well-being over time. TUS are used to understand how Canadian residents spend and manage their
...
conducted in 1986, and the most recent was completed in 2022. These surveys [@statisticscanada2022] collect data on respondents'
...
none of the variables used in this research are in the 2022
...
The Public Use Microdata Files (PUMFs) are released by Statistics Canada in two files:
...
The main file of the TUS compiles a large array of
...
ep_2015_unprocessed <- gss_e_2015 |>
  ...
  kable(
    ...
    longtable = FALSE, 
    align = c(rep("c", ncol(gss_m_2015[1:10, 1:6]))),
    caption = "\\label{tab:ep-2015-unprocessed}Visualization of the  first seven episodes of the record number 10041.") |>
  kable_styling(full_width = FALSE, 
                #latex_options=c("striped", "scale_down"), 
                latex_options=c("scale_down"),
                 font_size = 8) |>
...
all records from the TUSs from 1986 to 2022, that together
...
the unique active travel episode appears in Table
...
styled_ch03_table_01 <- stats_data |>
  kable(format = "latex", 
    format.args = list(big.mark = ",", decimal.mark = "."),
    booktabs = TRUE, longtable = FALSE, 
        caption = "\\label{tab:table-01}Descriptive statistics of the duration (in minutes) of episodes with active transport records.",  
...
In 2022, home location served as a central hub for most trips, with fewer than
...
median_duration_table <- median_duration_cma |>
  kable(
    ...
    format.args = list(big.mark = ",", decimal.mark = "."), 
    booktabs = TRUE, 
    caption = "\\label{tab:cma-durations}Differences in walking duration (in minutes) between provinces and population centre type.") |>
  kable_styling(full_width = FALSE, 
                #latex_options=c("striped", "scale_down"), 
                latex_options=c("scale_down"),
                 font_size = 8) |>
...
The work has been supported by funding from the Social Sciences and Humanities Research Council’s Partnership Grant: Mobilizing justice: towards evidence-based transportation equity policy.

# ORCID

- Bruno Dias dos Santos - https://orcid.org/0000-0002-6748-2038 
- Mahdis Moghadasi - https://orcid.org/0000-0003-1009-9369 
- Antonio Páez - https://orcid.org/0000-0001-6912-9919 


# Data availability statement

The {ActiveCA} R data package can be found and installed on [Github](https://github.com/dias-bruno/ActiveCA). 
